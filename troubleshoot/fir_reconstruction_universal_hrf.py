...
# Handle pilot data: directory is sub-P01, but files are named sub-01
if SUBJECT_ID == 'P01':
    FMRIPREP_DIR = f"{FMRIPREP_BASE}/sub-P01"  # Directory name: sub-P01
    FILE_PREFIX = "sub-01"  # File prefix: sub-01
    DERIVATIVE_PREFIX = "sub-P01"  # Derivative folder: sub-P01 (to distinguish from test sub-01)
    EVENT_DIR = f"{PILOT_DIR}/sub-P01/func"  # Events in pilot/sub-P01/func/
    ...
    EVENT_DIR = f"{PILOT_DIR}/sub-{SUBJECT_ID}/func"  # Events in pilot/sub-XX/func/
...
output_dir = Path(f"derivatives/{DERIVATIVE_PREFIX}/fir_reconstruction/{ROI_NAME}_universal_hrf")
...
for run in range(1, N_RUNS + 1):
    # Functional data path (pilot has res-2, test subjects don't)
    if SUBJECT_ID == 'P01':
        func_path = f"{FMRIPREP_DIR}/func/{FILE_PREFIX}_task-rsvp_run-{run}_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz"
    else:
        func_path = f"{FMRIPREP_DIR}/func/{FILE_PREFIX}_task-rsvp_run-{run}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz"
...
for held_out_color in range(N_COLORS):
    ...
    mean_reconstructed_hue = circular_mean_deg(all_reconstructed_hues)

    novel_color_results.append({
        'color': color_name,
        'reconstructed_hue': mean_reconstructed_hue,  # Mean prediction for visualization
...
ax.set_title(f'{ROI_NAME}: Reconstruction Results (All Runs)\nTrue hues and prediction clusters',
             fontsize=14, fontweight='bold', pad=20)
...
for color_idx in range(N_COLORS):
    ...
    ax.scatter([theta_true], [r_center], s=200, color=stim_rgb,
               edgecolor='k', linewidths=1.5, zorder=4, alpha=0.9)
    ...
    if len(preds):
        ...
        ax.scatter(thetas, r_jit, s=60, color=stim_rgb, alpha=0.5,
        ...
        if not np.isnan(mu_deg):
            ...
            r0, r1 = 0.60, 0.60 + 0.25*R
            ax.plot([mu_rad, mu_rad], [r0, r1], color=stim_rgb,
                    linewidth=3.0, alpha=0.9, zorder=2, solid_capstyle='round')

    # Add color label
    label_radius = 1.15
    ax.text(theta_true, label_radius, f'c{color_idx+1}',
            ha='center', va='center', fontsize=10, fontweight='bold',
            bbox=dict(boxstyle='circle,pad=0.3', facecolor='white', edgecolor='black', linewidth=1))

# Legend
from matplotlib.lines import Line2D
legend_elems = [
    Line2D([0],[0], marker='o', color='w', markerfacecolor='gray',
           markeredgecolor='k', label='True hue (stimulus)', markersize=12, linewidth=1.5),
    Line2D([0],[0], marker='o', color='w', markerfacecolor='gray',
           alpha=0.5, markeredgecolor='k', label='Predictions (6 runs)', markersize=8, linewidth=0.5),
    Line2D([0],[0], color='gray', lw=3, label='Circular mean vector')
]
ax.legend(handles=legend_elems, loc='upper right', bbox_to_anchor=(1.3, 1.15), fontsize=10)
...
for result in novel_color_results:
    ...
    # Novel color reconstruction (leave-one-color-out: single prediction per color)
    pred_hue = result['reconstructed_hue']
    pred_rgb = lab_hue_to_rgb(pred_hue)

    # Plot prediction (colored by predicted hue)
    ax2.scatter([np.deg2rad(pred_hue)], [r_pred_base], s=50,
               color=pred_rgb, alpha=0.8, edgecolor='k', linewidths=0.5, zorder=3)

    # Draw line from true to predicted
    ax2.plot([np.deg2rad(true_hue), np.deg2rad(pred_hue)],
             [r_center, r_pred_base],
