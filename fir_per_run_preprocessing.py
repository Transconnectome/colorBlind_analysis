#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
fir_per_run_preprocessing.py
----------------------------------
Per-run FIR GLM with preprocessing parameter testing

This extends fir_per_run_simple.py to test different preprocessing combinations
based on literature recommendations for FIR GLM and MVPA.

PREPROCESSING CONFIGURATIONS (Literature-based):

Config 1: Baseline (current)
  - Smoothing: 0 mm
  - High-pass: None
  - Drift: linear detrend
  - Motion: 6 params
  - Rationale: Preserve fine-grained patterns, minimal preprocessing

Config 2: SPM default
  - Smoothing: 4 mm
  - High-pass: 1/128s
  - Drift: linear detrend
  - Motion: 6 params
  - Rationale: Standard fMRI preprocessing (Friston et al.)

Config 3: Conservative MVPA
  - Smoothing: 6 mm
  - High-pass: 1/128s
  - Drift: linear detrend
  - Motion: 6 params
  - Rationale: Balance SNR and spatial specificity (Op de Beeck 2010)

Config 4: No-smoothing MVPA
  - Smoothing: 0 mm
  - High-pass: 1/128s
  - Drift: linear detrend
  - Motion: 6 params
  - Rationale: Mumford et al. (2012) recommendation for MVPA

Config 5: Polynomial drift
  - Smoothing: 4 mm
  - High-pass: None
  - Drift: polynomial (order 2)
  - Motion: 6 params
  - Rationale: Alternative drift modeling without high-pass filtering

Config 6: Minimal preprocessing
  - Smoothing: 0 mm
  - High-pass: None
  - Drift: None (no detrending)
  - Motion: 6 params
  - Rationale: Motion correction only, preserve all signals

Usage:
    python fir_per_run_preprocessing.py --roi V1 --subject 01 --strategy flatten --config 2

Created: 2025-01-20
"""

import sys
import os
import argparse
import numpy as np
import pandas as pd
import nibabel as nib
from pathlib import Path
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
from nilearn import image as nimg
from nilearn.signal import clean

try:
    from nilearn.maskers import NiftiMasker
except ImportError:
    from nilearn.input_data import NiftiMasker

from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA
from scipy.stats import zscore
from scipy.ndimage import gaussian_filter

# ============================================================================
# Preprocessing Configurations
# ============================================================================

PREPROC_CONFIGS = {
    1: {
        'name': 'baseline',
        'smoothing_fwhm': 0,
        'high_pass': None,
        'drift_model': 'linear',
        'motion_confounds': True,
        'description': 'Current baseline - minimal preprocessing'
    },
    2: {
        'name': 'spm_default',
        'smoothing_fwhm': 4,
        'high_pass': 1/128,  # Hz
        'drift_model': 'linear',
        'motion_confounds': True,
        'description': 'SPM default preprocessing'
    },
    3: {
        'name': 'conservative_mvpa',
        'smoothing_fwhm': 6,
        'high_pass': 1/128,
        'drift_model': 'linear',
        'motion_confounds': True,
        'description': 'Conservative MVPA - balance SNR and specificity'
    },
    4: {
        'name': 'nosmooth_mvpa',
        'smoothing_fwhm': 0,
        'high_pass': 1/128,
        'drift_model': 'linear',
        'motion_confounds': True,
        'description': 'No-smoothing MVPA (Mumford et al. 2012)'
    },
    5: {
        'name': 'poly_drift',
        'smoothing_fwhm': 4,
        'high_pass': None,
        'drift_model': 'poly2',
        'motion_confounds': True,
        'description': 'Polynomial drift without high-pass'
    },
    6: {
        'name': 'minimal',
        'smoothing_fwhm': 0,
        'high_pass': None,
        'drift_model': None,
        'motion_confounds': True,
        'description': 'Minimal - motion correction only'
    },
}

# ============================================================================
# Configuration
# ============================================================================

LABEL2HUE_DEG_TEST = {
    'color_1': 0.0,
    'color_2': 45.0,
    'color_3': 90.0,
    'color_4': 135.0,
    'color_5': 180.0,
    'color_6': 225.0,
    'color_7': 270.0,
    'color_8': 315.0,
}

TR = 1.5
N_RUNS = 6
N_COLORS = 8
VOLS_TO_DROP = 4
FIR_DELAYS = np.arange(8)  # 0-10.5s

# ============================================================================
# Helper Functions
# ============================================================================

def diag_linear_predict(train_X, train_y, test_X):
    """Diagonal LDA"""
    classes = np.unique(train_y)
    means = np.stack([train_X[train_y==c].mean(axis=0) for c in classes])
    vars_  = np.stack([train_X[train_y==c].var(axis=0) + 1e-8 for c in classes])

    ll = []
    for k in range(len(classes)):
        ll_k = -0.5 * (
            np.log(2*np.pi*vars_[k]).sum() +
            ((test_X - means[k])**2 / vars_[k]).sum(axis=1)
        )
        ll.append(ll_k)
    ll = np.stack(ll, axis=1)
    return classes[ll.argmax(axis=1)]

def build_color_fir_design(events, n_scans, tr, fir_delays, n_colors=8):
    """Build color-specific FIR design matrix"""
    n_delays = len(fir_delays)
    X = np.zeros((n_scans, n_colors * n_delays))

    for color_idx in range(1, n_colors + 1):
        color_name = f'color_{color_idx}'
        color_events = events[events['trial_type'] == color_name]

        for onset in color_events['onset'].values:
            onset_tr = int(np.round(onset / tr))
            for i, delay in enumerate(fir_delays):
                tr_idx = onset_tr + delay
                if 0 <= tr_idx < n_scans:
                    col_idx = (color_idx - 1) * n_delays + i
                    X[tr_idx, col_idx] = 1.0
    return X

def add_polynomial_drift(n_scans, order=2):
    """Create polynomial drift regressors"""
    t = np.arange(n_scans)
    t_normalized = (t - t.mean()) / t.std()

    drift_regressors = []
    for i in range(1, order + 1):
        drift_regressors.append(t_normalized ** i)

    return np.column_stack(drift_regressors)

def preprocess_data(func_img, confounds_df, config, tr, vols_to_drop=4):
    """
    Preprocess functional data according to config

    Parameters:
    -----------
    func_img : nibabel image
        Functional data
    confounds_df : pandas DataFrame
        Confounds timeseries
    config : dict
        Preprocessing configuration
    tr : float
        Repetition time
    vols_to_drop : int
        Number of initial volumes to drop

    Returns:
    --------
    func_clean : ndarray
        Cleaned functional data (scans, voxels)
    """

    # Drop initial volumes
    if vols_to_drop > 0:
        func_img = nimg.index_img(func_img, slice(vols_to_drop, None))
        confounds_df = confounds_df.iloc[vols_to_drop:]

    # Smoothing
    if config['smoothing_fwhm'] > 0:
        func_img = nimg.smooth_img(func_img, fwhm=config['smoothing_fwhm'])

    # Extract data with masker
    from nilearn.maskers import NiftiMasker
    masker = NiftiMasker(standardize=False)
    func_data = masker.fit_transform(func_img)

    # Prepare confounds
    confounds_list = []

    # Motion parameters
    if config['motion_confounds']:
        motion = confounds_df[['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z']].values
        confounds_list.append(motion)

    # Drift regressors (if using polynomial instead of high-pass)
    if config['drift_model'] == 'poly2':
        drift = add_polynomial_drift(func_data.shape[0], order=2)
        confounds_list.append(drift)
    elif config['drift_model'] == 'poly3':
        drift = add_polynomial_drift(func_data.shape[0], order=3)
        confounds_list.append(drift)

    # Combine confounds
    if len(confounds_list) > 0:
        confounds_array = np.column_stack(confounds_list)
    else:
        confounds_array = None

    # Determine detrending
    if config['drift_model'] in ['poly2', 'poly3']:
        # Already added polynomial regressors, don't use linear detrend
        do_detrend = False
    elif config['drift_model'] == 'linear':
        do_detrend = True
    else:  # None
        do_detrend = False

    # Clean data
    func_clean = clean(
        func_data,
        confounds=confounds_array,
        detrend=do_detrend,
        standardize=False,
        standardize_confounds=True,
        high_pass=config['high_pass'],
        t_r=tr
    )

    return func_clean

# ============================================================================
# Parse Arguments
# ============================================================================

parser = argparse.ArgumentParser()
parser.add_argument('--subject', type=str, required=True)
parser.add_argument('--roi', type=str, required=True)
parser.add_argument('--strategy', type=str, default='flatten',
                    choices=['flatten', 'average', 'delay3', 'delay4', 'delay5'])
parser.add_argument('--config', type=int, default=1, choices=[1, 2, 3, 4, 5, 6],
                    help='Preprocessing configuration (1-6)')
parser.add_argument('--use-pca', action='store_true')
parser.add_argument('--n-components', type=int, default=6)
parser.add_argument('--timestamp', type=str, default=None)
args = parser.parse_args()

SUBJECT_ID = args.subject
ROI_NAME = args.roi
STRATEGY = args.strategy
CONFIG_ID = args.config
USE_PCA = args.use_pca
N_PCA_COMPONENTS = args.n_components

config = PREPROC_CONFIGS[CONFIG_ID]

# ============================================================================
# Paths
# ============================================================================

FMRIPREP_BASE = "/storage/connectome/haba6030/fmriprep_out"
EVENT_DIR = "/storage/connectome/haba6030/colorBlind_dataOct"

FMRIPREP_DIR = f"{FMRIPREP_BASE}/sub-{SUBJECT_ID}"
FILE_PREFIX = f"sub-{SUBJECT_ID}"
EVENTS_DIR = f"{EVENT_DIR}/sub-{SUBJECT_ID}/func"

# Output
from datetime import datetime
timestamp = args.timestamp or datetime.now().strftime("%Y%m%d_%H%M%S")

pca_suffix = f"_pca{N_PCA_COMPONENTS}" if USE_PCA else ""
output_dir = Path(f"derivatives/fir_preprocessing/sub-{SUBJECT_ID}/{timestamp}_{ROI_NAME}_{STRATEGY}_cfg{CONFIG_ID}{pca_suffix}")
output_dir.mkdir(parents=True, exist_ok=True)
fig_dir = output_dir / 'figures'
fig_dir.mkdir(exist_ok=True)

print("="*80)
print("Per-Run FIR GLM with Preprocessing Testing")
print("="*80)
print(f"Subject: {SUBJECT_ID}")
print(f"ROI: {ROI_NAME}")
print(f"Strategy: {STRATEGY}")
print(f"Config {CONFIG_ID}: {config['name']}")
print(f"  - Smoothing: {config['smoothing_fwhm']} mm")
print(f"  - High-pass: {config['high_pass']} Hz" if config['high_pass'] else "  - High-pass: None")
print(f"  - Drift model: {config['drift_model']}")
print(f"  - Motion confounds: {config['motion_confounds']}")
print(f"Use PCA: {USE_PCA}" + (f" (n={N_PCA_COMPONENTS})" if USE_PCA else ""))
print(f"Output: {output_dir}")
print()
sys.stdout.flush()

# ============================================================================
# Load ROI Mask
# ============================================================================

print(f"[1/5] Loading ROI mask")

roi_path = f"derivatives/sub-{SUBJECT_ID}/roi_pipeline/{ROI_NAME}_mask_thr50_intnearest_binTrue_masknone_gmTrue_subjFalse.nii.gz"

if not os.path.exists(roi_path):
    print(f"ERROR: {roi_path} not found")
    sys.exit(1)

roi_img = nib.load(roi_path)
n_voxels = int(np.sum(roi_img.get_fdata() > 0))
print(f"  ROI voxels: {n_voxels}")

masker = NiftiMasker(mask_img=roi_img, standardize=False)
masker.fit()
print()

# ============================================================================
# Load and Preprocess Data
# ============================================================================

print(f"[2/5] Loading and preprocessing {N_RUNS} runs")

all_func_data = []
events_list = []

for run in range(1, N_RUNS + 1):
    func_path = f"{FMRIPREP_DIR}/func/{FILE_PREFIX}_task-rsvp_run-{run}_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz"
    confounds_path = f"{FMRIPREP_DIR}/func/{FILE_PREFIX}_task-rsvp_run-{run}_desc-confounds_timeseries.tsv"
    events_path = f"{EVENTS_DIR}/{FILE_PREFIX}_task-rsvp_run-{run}_events.tsv"

    func_img = nib.load(func_path)
    confounds = pd.read_csv(confounds_path, sep='\t')

    # Preprocess with selected config
    func_clean = preprocess_data(func_img, confounds, config, TR, VOLS_TO_DROP)

    # Apply ROI mask
    func_roi = masker.transform(nimg.index_img(func_img, slice(VOLS_TO_DROP, None)))

    # If smoothing, need to smooth the ROI data
    if config['smoothing_fwhm'] > 0:
        # Reconstruct 4D image for smoothing
        func_4d = masker.inverse_transform(func_roi)
        func_smooth = nimg.smooth_img(func_4d, fwhm=config['smoothing_fwhm'])
        func_roi = masker.transform(func_smooth)

    # Clean the ROI data
    motion_df = confounds[['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z']]
    if VOLS_TO_DROP > 0:
        motion_df = motion_df.iloc[VOLS_TO_DROP:]

    confounds_list = []
    if config['motion_confounds']:
        confounds_list.append(motion_df.values)

    if config['drift_model'] in ['poly2', 'poly3']:
        order = int(config['drift_model'][-1])
        drift = add_polynomial_drift(func_roi.shape[0], order=order)
        confounds_list.append(drift)

    confounds_array = np.column_stack(confounds_list) if confounds_list else None
    do_detrend = (config['drift_model'] == 'linear')

    func_clean = clean(
        func_roi,
        confounds=confounds_array,
        detrend=do_detrend,
        standardize=False,
        standardize_confounds=True,
        high_pass=config['high_pass'],
        t_r=TR
    )

    all_func_data.append(func_clean)

    # Load events
    events = pd.read_csv(events_path, sep='\t')
    if VOLS_TO_DROP > 0:
        events['onset'] = events['onset'] - (VOLS_TO_DROP * TR)
        events = events[events['onset'] >= 0].reset_index(drop=True)
    events_list.append(events)

    print(f"  Run {run}: {func_clean.shape[0]} scans, {len(events)} events")

print()

# ============================================================================
# Step 1: Per-Run FIR GLM
# ============================================================================

print(f"[3/5] Per-run FIR GLM")
print(f"  Extracting FIR amplitudes: (runs={N_RUNS}, colors={N_COLORS}, voxels={n_voxels}, delays={len(FIR_DELAYS)})")

amplitudes = np.zeros((N_RUNS, N_COLORS, n_voxels, len(FIR_DELAYS)))

for run_idx in range(N_RUNS):
    print(f"  Run {run_idx+1}/{N_RUNS}...", end='')

    y = all_func_data[run_idx]
    n_scans = y.shape[0]

    X = build_color_fir_design(events_list[run_idx], n_scans, TR, FIR_DELAYS, N_COLORS)

    betas = np.linalg.pinv(X) @ y
    betas_reshaped = betas.reshape(N_COLORS, len(FIR_DELAYS), n_voxels)
    amplitudes[run_idx] = np.transpose(betas_reshaped, (0, 2, 1))

    print(f" mean={np.mean(amplitudes[run_idx]):.4f}, std={np.std(amplitudes[run_idx]):.4f}")

print(f"\n  FIR amplitudes shape: {amplitudes.shape}")
print()

# ============================================================================
# Step 2: Apply Strategy
# ============================================================================

print(f"[4/5] Applying strategy: {STRATEGY}")

if STRATEGY == 'flatten':
    features = amplitudes.reshape(N_RUNS, N_COLORS, -1)
    print(f"  Flattened all delays: {features.shape}")

elif STRATEGY == 'average':
    features = np.mean(amplitudes, axis=3)
    print(f"  Averaged across delays: {features.shape}")

elif STRATEGY.startswith('delay'):
    delay_idx = int(STRATEGY[-1])
    features = amplitudes[:, :, :, delay_idx]
    print(f"  Using delay {delay_idx} ({delay_idx*TR:.1f}s): {features.shape}")

print()

# ============================================================================
# Step 3: Z-score
# ============================================================================

print(f"  Z-scoring per run per feature...")

if STRATEGY == 'flatten':
    z_features = np.zeros_like(features)
    n_voxels_delays = features.shape[2]

    for run_idx in range(N_RUNS):
        for vd_idx in range(n_voxels_delays):
            vals = features[run_idx, :, vd_idx]
            if np.std(vals) > 0:
                z_features[run_idx, :, vd_idx] = zscore(vals)
            else:
                z_features[run_idx, :, vd_idx] = 0
else:
    z_features = np.zeros_like(features)

    for run_idx in range(N_RUNS):
        for voxel_idx in range(n_voxels):
            vals = features[run_idx, :, voxel_idx]
            if np.std(vals) > 0:
                z_features[run_idx, :, voxel_idx] = zscore(vals)
            else:
                z_features[run_idx, :, voxel_idx] = 0

print(f"  Z-scored features: {z_features.shape}")
print(f"  Mean: {np.mean(z_features):.6f}, Std: {np.std(z_features):.6f}")
print()

# ============================================================================
# Step 4: Classification
# ============================================================================

print(f"[5/5] Classification (leave-one-run-out)")

results = []

for test_run in range(N_RUNS):
    train_runs = [r for r in range(N_RUNS) if r != test_run]

    X_train = z_features[train_runs].reshape(-1, z_features.shape[2])
    y_train = np.tile(np.arange(N_COLORS), len(train_runs))

    X_test = z_features[test_run]
    y_test = np.arange(N_COLORS)

    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)

    if USE_PCA:
        pca = PCA(n_components=N_PCA_COMPONENTS)
        X_train_final = pca.fit_transform(X_train_scaled)
        X_test_final = pca.transform(X_test_scaled)
    else:
        X_train_final = X_train_scaled
        X_test_final = X_test_scaled

    y_pred = diag_linear_predict(X_train_final, y_train, X_test_final)
    acc = (y_pred == y_test).mean()

    results.append({'test_run': test_run+1, 'accuracy': acc})
    print(f"  Test run {test_run+1}: {acc:.3f} ({acc*100:.1f}%)")

mean_acc = np.mean([r['accuracy'] for r in results])
print(f"\n  Mean accuracy: {mean_acc:.3f} ({mean_acc*100:.1f}%)")
print(f"  Chance: {1/N_COLORS:.3f} ({100/N_COLORS:.1f}%)")
print()

# ============================================================================
# Save Results
# ============================================================================

print("Saving results...")

import json
summary = {
    'subject': SUBJECT_ID,
    'roi': ROI_NAME,
    'strategy': STRATEGY,
    'config_id': CONFIG_ID,
    'config_name': config['name'],
    'preprocessing': {
        'smoothing_fwhm': config['smoothing_fwhm'],
        'high_pass': config['high_pass'],
        'drift_model': config['drift_model'],
        'motion_confounds': config['motion_confounds'],
    },
    'use_pca': USE_PCA,
    'n_pca_components': N_PCA_COMPONENTS if USE_PCA else None,
    'n_voxels': int(n_voxels),
    'n_delays': len(FIR_DELAYS),
    'mean_accuracy': float(mean_acc),
    'per_run_results': results,
}

with open(output_dir / 'summary.json', 'w') as f:
    json.dump(summary, f, indent=2)

np.save(output_dir / 'amplitudes.npy', amplitudes)
np.save(output_dir / 'z_features.npy', z_features)

print()
print("="*80)
print("Complete!")
print("="*80)
print(f"\nResults:")
print(f"  Config {CONFIG_ID}: {config['name']}")
print(f"  Strategy: {STRATEGY}")
print(f"  Classification: {mean_acc:.1%}")
print(f"  Saved to: {output_dir}")
print()
