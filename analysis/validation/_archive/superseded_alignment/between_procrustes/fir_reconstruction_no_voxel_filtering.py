...
----------------------------------
MODIFIED VERSION: Disables problematic voxel filtering for between-subject analysis

KEY MODIFICATIONS FROM ORIGINAL:
1. KEEPS R² filtering (top 50% voxels) - ensures good HRF estimation quality
2. DISABLES zero-variance/NaN filtering - prevents voxel count heterogeneity
3. Handles NaN/zero-variance during z-scoring with mean imputation/centering
4. Output directory suffix: "_unfiltered" to distinguish from original

PURPOSE: Generate baseline results with consistent voxel sets across subjects
for between-subject Procrustes analysis with ANOVA voxel selection.
...
# ⚠️ MODIFIED VERSION CONFIGURATION
ENABLE_VOXEL_FILTERING = False  # DISABLED for between-subject analysis
OUTPUT_DIR_SUFFIX = "_unfiltered"  # Distinguish from original baseline
...
if args.output_dir:
    ...
    if DRIFT_MODEL == 'per_run':
        # ⚠️ MODIFIED: Add OUTPUT_DIR_SUFFIX to distinguish unfiltered results
        output_dir = Path(f"analysis/phase1_preprocess_decoding/{DATASET}/results/baseline_decoding/fixed_perRun{OUTPUT_DIR_SUFFIX}/sub-{SUBJECT_ID}/{ROI_NAME}")
        output_dir.mkdir(parents=True, exist_ok=True)
        output_dir = str(output_dir)
        fig_dir = str(output_dir) + '/figures_'
        print("🔧 MODIFIED VERSION: No voxel filtering (for between-subject analysis)")
    else:
        ...
        condition_name = f"{nz_short}_{mo_short}{OUTPUT_DIR_SUFFIX}"  # ⚠️ MODIFIED: Add suffix
...
# ⚠️ MODIFIED: Keep ALL voxels, handle NaN/zero-variance during z-scoring
amplitudes_z = np.zeros_like(amplitudes_raw)

# Track how many voxels have issues (for reporting only, not filtering)
n_nan_voxels = 0
n_zero_var_voxels = 0
...
for run_idx in range(N_RUNS):
    for voxel_idx in range(n_voxels_selected):
        ...
        # Handle NaN: replace with run-mean
        if np.any(np.isnan(amp)):
            amp = np.nan_to_num(amp, nan=np.nanmean(amp))
            n_nan_voxels += 1

        # Handle zero variance: keep raw values (z-score = 0)
        if np.std(amp) > 1e-10:
            amplitudes_z[run_idx, :, voxel_idx] = zscore(amp)
        else:
            amplitudes_z[run_idx, :, voxel_idx] = amp - np.mean(amp)
            n_zero_var_voxels += 1

# Report issues but DO NOT filter
if n_nan_voxels > 0:
    print(f"  Voxels with NaN (handled with mean imputation): {n_nan_voxels}/{n_voxels_selected}")
if n_zero_var_voxels > 0:
    print(f"  Voxels with zero variance (handled with mean-centering): {n_zero_var_voxels}/{n_voxels_selected}")

print(f"  ✅ All {n_voxels_selected} voxels retained (no filtering)")
