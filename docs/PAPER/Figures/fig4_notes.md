...
## Post-feedback revision — 2026-05-12

NotebookLM 질의 기반(Schütt 2021 / Kotani 2025 등) 다음 변경:

1. **Panel A에 Machado dashed line 복원** — 2-component 단독 표시 → 2-component(solid) + Machado(dashed, alpha=0.6) competing model 비교. 이전 notes에는 "Machado dashed line" 명시되어 있었으나 실제 코드는 2-component만 그리고 있었음 (notes/code 불일치 해결).

2. **Panel B 4-bar 모델 비교** — 2-component 단독 2-bar → (2-comp + Machado) × (sub-08, sub-09) = 4-bar grouped chart. 2-comp는 solid color, Machado는 hatched/lighter alpha.

3. **Chance band (gray shaded)** — 각 subject별 label-permutation null distribution의 mean ± 2σ를 회색 띠로 표시. Schütt 2021 Fig 1C/7 noise ceiling 컨벤션의 직접 대응물 (실제 noise ceiling은 inter-HC 신뢰도가 필요해 본 분석에선 chance reference만 표시).

4. **Theoretical ceiling line** — ρ = 1.0에 dotted line + "ρ=1 (ceiling)" 라벨. NotebookLM: "절대적 기준 시각화".

5. **Δρ pairwise difference arrows** — 각 subject 위에 2-comp ↔ Machado bar를 화살표로 연결하고 Δρ 값 표시 (sub-08 Δρ=+0.26, sub-09 Δρ=−0.07). Schütt 2021 Fig 1C의 pairwise difference arrow network 컨벤션. p-value/별표 단독으로 model 우위를 선언하지 말 것 (NotebookLM Don't).

### 수치 (NotebookLM 답변 인용 후 실제 데이터로 재검증)

| Subject | 2-component | Machado | Δρ (2c − Machado) |
|---|---|---|---|
| Sub-08 deutan | ρ=0.881 (** p=0.004) | ρ=0.619 (n.s. p=0.058) | **+0.26** |
| Sub-09 protan | ρ=0.690 (* p=0.035) | ρ=0.762 (* p=0.018) | **−0.07** |

Sub-09 raw ρ에서 Machado가 약간 우위. 2-component의 sub-09 우위 주장은 LOCO ρ만이 아니라 dual-criterion (LOCO + exact pre-image) + biological grounding에 근거함 — manuscript 본문에서 명시 필요. 

### Deferred (향후 개선)

- **Parameter schematic** (β_s, β_c → color wheel distortion 시각화): Kotani 2025 Fig 11 식의 "params → wheel distortion" schematic. 현재 Panel C parameter landscape가 정량적으로 보여주나, 그 의미를 schematic으로 직관화하는 사이드 패널 필요. 별도 supplementary로 검토.
