...
class Config:
    ...
    STORAGE_DIR = '/storage/connectome/haba6030/fmriprep_out'
    ...
    @property
    def output_dir(self):
        return os.path.join(self.STORAGE_DIR, f'sub-{self.SUB_ID}')
    ...
    def get_func_img_path(self, run):
        return os.path.join(
            self.fmriprep_dir,
            f'sub-{self.FILE_PRE}_task-rsvp_run-{run}_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz'
        )

    def get_event_file_path(self, run):
        return os.path.join(
            self.EVENT_DIR,
            f'sub-{self.SUB_ID}/func/sub-{self.FILE_PRE}_task-rsvp_run-{run}_events.tsv'
        )

    def get_confound_file_path(self, run):
        return os.path.join(
            self.fmriprep_dir,
            f'sub-{self.FILE_PRE}_task-rsvp_run-{run}_desc-confounds_timeseries.tsv'
        )
