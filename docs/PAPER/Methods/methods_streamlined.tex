...
CVD color distortion was modeled as a two-component hue dilation in Stockman opponent-color space (Equation~\ref{eq:2comp}):
...
where $h$ is the opponent-hue angle of the stimulus, and $\theta_{\rm conf}$ is the CVD-subtype confusion axis in Stockman space (protan: $16^\circ$; deutan: $150^\circ$). $\beta_s$ weights distortion along the S-cone opponent axis, motivated by behavioral evidence that CVD perceptual errors concentrate near S-cone confusion loci \cite{emery2021}. $\beta_c$ weights distortion along the CVD-type confusion axis, capturing the partial cortical compensation documented in V2/V3 \cite{tregillus2021}. Opponent-hue angles $h$ were derived from CIELab coordinates using the Machado simulation framework at zero spectral shift \cite{machado2009}. The perceived hue was $\theta' = (\theta + \delta\theta) \bmod 360^\circ$. Parameters were estimated by a grid search over $\beta_s \in [0^\circ, 50^\circ]$ and $\beta_c \in [-50^\circ, 50^\circ]$ (26 $\times$ 51 = 1,326 combinations, $2^\circ$ resolution). The pair maximizing hV4 LOCO Pearson $r$ was retained. Permutation significance testing followed the same procedure as for the encoding model.
