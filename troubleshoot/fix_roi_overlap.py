...
def create_combined_rois(func_mask_img, output_dir):
    ...
    # Early Visual (V1 + V2)
    v1_path = os.path.join(cfg.roi_dir, f'sub-{cfg.FILE_PRE}_V1_mask.nii.gz')
    v2_path = os.path.join(cfg.roi_dir, f'sub-{cfg.FILE_PRE}_V2_mask.nii.gz')
    ...
    if os.path.exists(v1_path) and os.path.exists(v2_path):
        ...
        out_path = os.path.join(output_dir, f'sub-{cfg.FILE_PRE}_EarlyVisual_fixed_mask.nii.gz')
        nib.save(early_img, out_path)
        print(f"  Saved: {out_path}")

    # Ventral (V3 + hV4)
    v3_path = os.path.join(cfg.roi_dir, f'sub-{cfg.FILE_PRE}_V3_mask.nii.gz')
    v4_path = os.path.join(cfg.roi_dir, f'sub-{cfg.FILE_PRE}_hV4_mask.nii.gz')
    ...
    if os.path.exists(v3_path) and os.path.exists(v4_path):
        ...
        out_path = os.path.join(output_dir, f'sub-{cfg.FILE_PRE}_Ventral_fixed_mask.nii.gz')
...
def main():
    ...
    print(f"file Pre: sub-{cfg.FILE_PRE}")
    ...
    for roi_name in roi_names:
        roi_path = os.path.join(
            cfg.roi_dir,
            f'sub-{cfg.FILE_PRE}_{roi_name}_mask.nii.gz'
        )
