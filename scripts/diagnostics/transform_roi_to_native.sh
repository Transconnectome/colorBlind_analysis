#!/bin/bash
#
# Transform ROI from MNI atlas to native BOLD space
#
# Purpose: Verify ROI fitting in native functional space independent of MNI registration quality
#
# Pipeline:
#   1. MNI ROI → T1w native (using ANTs inverse warp)
#   2. T1w ROI → BOLD native (using fMRIPrep transform)
#   3. Visual QC overlay
#   4. Functional sanity check
#
...
OUTPUT_DIR="/scratch/connectome/haba6030/colorBlind/derivatives/native_space_roi"
...
# Transform files (original_v3 uses acq-mprage)
MNI_TO_T1W="${ANAT_DIR}/sub-${SUBJECT}_acq-mprage_from-MNI152NLin2009cAsym_to-T1w_mode-image_xfm.h5"
BOLDREF_TO_T1W="${FUNC_DIR}/sub-${SUBJECT}_task-rsvp_run-${RUN}_from-boldref_to-T1w_mode-image_xfm.txt"

# Reference images (original_v3 uses acq-mprage for T1w)
...
# Check BOLDREF (prefer coreg_boldref, fallback to T1w space BOLD)
if [ -f "$BOLDREF" ]; then
    echo "✓ Found: $(basename $BOLDREF)"
    BOLD_NATIVE="$BOLDREF"
elif [ -f "$BOLD_T1W" ]; then
    echo "⚠ Using T1w space BOLD as reference: $(basename $BOLD_T1W)"
    BOLD_NATIVE="$BOLD_T1W"
else
    echo "✗ MISSING: Native BOLD reference"
    echo "  Tried: $BOLDREF"
    echo "  Tried: $BOLD_T1W"
    MISSING_FILES=$((MISSING_FILES + 1))
fi
...
# ============================================================================
# Step 4: T1w ROI → BOLD Native
# ============================================================================

echo "🔄 Step 4: Transforming T1w ROI → BOLD native..."
echo "--------------------------------------------------------------------------------"

ROI_BOLD="${OUTPUT_DIR}/sub-${SUBJECT}/sub-${SUBJECT}_${ROI}_space-bold_run-${RUN}.nii.gz"

# Check if we need additional transform (only if using coreg_boldref)
if [ -f "$BOLDREF_TO_T1W" ] && [ "$BOLD_NATIVE" = "$BOLDREF" ]; then
    echo "Using BOLD→T1w transform (inverse applied)..."
    ...
    echo "Using T1w space BOLD (no additional transform needed)..."

    # If BOLD is already in T1w space, just resample to match BOLD grid
    ${APPTAINER} exec --cleanenv \
        -B /storage:/storage -B /scratch:/scratch "${IMG}" \
        antsApplyTransforms -d 3 \
            -i "$ROI_T1W" \
...
# Verify output
if [ ! -f "$ROI_BOLD" ]; then
    echo "❌ Failed to create BOLD-native ROI"
    exit 1
fi

VOX_BOLD=$(fslstats "$ROI_BOLD" -V | awk '{print $1}')
MAX_BOLD=$(fslstats "$ROI_BOLD" -R | awk '{print $2}')
echo "✓ BOLD-native ROI created: ${VOX_BOLD} voxels, max=${MAX_BOLD}"

echo ""

# ============================================================================
# Step 5: Create Binary Mask (threshold > 20)
# ============================================================================

echo "🎭 Step 5: Creating binary mask (threshold=20)..."
echo "--------------------------------------------------------------------------------"

ROI_MASK="${OUTPUT_DIR}/sub-${SUBJECT}/sub-${SUBJECT}_${ROI}_space-bold_run-${RUN}_mask.nii.gz"

fslmaths "$ROI_BOLD" -thr 20 -bin "$ROI_MASK"
...
# ============================================================================
# Step 6: Generate QC Overlay Image
# ============================================================================

echo "📊 Step 6: Generating QC overlay image..."
echo "--------------------------------------------------------------------------------"

QC_PNG="${OUTPUT_DIR}/sub-${SUBJECT}/QC_sub-${SUBJECT}_${ROI}_run-${RUN}_overlay.png"

# Use FSL's slicer for quick QC
slicer "$BOLD_NATIVE" "$ROI_BOLD" \
    -a "$QC_PNG"
...
echo "  T1w native:  ${VOX_T1W} voxels (${MAX_T1W} max intensity)"
echo "  BOLD native: ${VOX_BOLD} voxels (${MAX_BOLD} max intensity)"
echo "  Binary mask: ${VOX_MASK} voxels (threshold > 20)"
echo ""
echo "Output files:"
echo "  T1w ROI:     $ROI_T1W"
echo "  BOLD ROI:    $ROI_BOLD"
echo "  Binary mask: $ROI_MASK"
echo "  QC overlay:  $QC_PNG"
echo ""

# Interpretation
if [ "$VOX_MASK" -gt 100 ]; then
    echo "✅ SUCCESS: ROI successfully transformed to native BOLD space"
    echo "   → Analysis can proceed in native space"
...
echo "  3. If successful, run functional analysis in native space"
echo ""
