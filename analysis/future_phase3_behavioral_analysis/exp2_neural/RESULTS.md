...
**Why LOCO and SRM diverge (interpretation)**: LOCO measures alignment of the hue→voxel *forward
tuning* to canonical hue angles (the perceptual target the filter restores). SRM measures raw 8-color
*geometry* similarity to HC, who viewed *unfiltered* canonical colors — so changing the on-screen
colors necessarily perturbs geometry even when the intended perceptual mapping is restored. The two
metrics answer different questions; the project's framework treats LOCO as primary.
