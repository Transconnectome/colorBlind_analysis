...
By contrast, the retinal-family comparison model carries one free parameter along a fixed profile. Its gain rescales the Machado cone shift as a whole, so the relative weight of its S-cone-axis and confusion-axis content is set by the cone model rather than fitted (Supplementary~\cref{app:retinal_family}), and it reached the gain boundary in the deutan participant ($g = 3.0$). Scored on the same held-out composite, its protan fit reached $\overline{L}_{\rm test} = -0.86$ against $-1.54$ for the 2-component model (Supplementary~\cref{app:retinal_family}).
