...
def train_hc_model(args):
    ...
    if len(hc_reconstruction_results) > 0:
        ...
        if np.mean(all_errors) < 90.0:
            print(f"  ✅ Common W is better than chance!")
        else:
            print(f"  ⚠️  WARNING: Common W is not better than chance!")

        if np.mean(all_accs) > 0.125:
            print(f"  ✅ Classification better than chance!")
        else:
            print(f"  ⚠️  WARNING: Classification not better than chance!")
...
def test_cvd_subjects(args):
    ...
    for sub_id in cvd_subjects:
        ...
        for run_idx in range(n_runs):
            ...
            # NO ALIGNMENT - Direct reconstruction with HC W
            X_test = pattern  # (n_colors, min_voxels)

            # Estimate channels using truncated W (no alignment)
            C_est = np.linalg.pinv(W_truncated.T @ W_truncated) @ W_truncated.T @ X_test.T
            # C_est: (6, n_colors)
            ...
            valid_runs += 1
            print(f"    Run {run_idx+1}: processed (no alignment)")
        ...
        if len(reconstruction_errors) == 0:
            ...
            print(f"    Reconstruction error (no alignment): {mean_error:.1f}°")
    ...
    # Save CSV (no alignment version)
    df = pd.DataFrame([
        {'subject_id': r['subject_id'], 'mean_error': r['mean_error']}
        for r in cvd_results
    ])

    csv_path = output_dir / 'cvd_reconstruction_errors_noalign.csv'
    df.to_csv(csv_path, index=False)
    print(f"  Saved: {csv_path}")
    ...
    for result in cvd_results:
        ...
        plt.tight_layout()
        wheel_path = output_dir / f'cvd_circular_wheel_noalign_sub-{result["subject_id"]}.png'
        plt.savefig(wheel_path, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    print(f"\n{'='*70}")
    print(f"CVD TESTING COMPLETE (NO ALIGNMENT)")
    print(f"{'='*70}\n")

    print(f"\nSummary:")
    print(f"{'Subject':<10} {'Error (no-align)':<20}")
    print(f"{'-'*35}")
    for r in cvd_results:
        if not np.isnan(r['mean_error']):
            print(f"sub-{r['subject_id']:<6} {r['mean_error']:>18.1f}°")

    print(f"\n{'='*70}")
    print(f"NOTE:")
    print(f"  Compare with aligned version to assess alignment benefit")
    print(f"  Files saved with '_noalign' suffix")
    print(f"{'='*70}")
