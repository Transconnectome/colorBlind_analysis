...
#### 1. W의 동일성은 직접 검증하지 못함

**현재 결과:**
- CVD + HC W (aligned): 5-10° error
- CVD + HC W (no-align): 84-96° error

**한계:**
이 결과는 Procrustes alignment 후 HC W가 "작동한다"는 것만 보여줌. **W matrix 자체가 HC-CVD 간 동일한지는 직접 검증하지 못했음.**

현재 필터 설계 로직:
```
Input color 조정 (Procrustes) → HC W 적용 → Reconstruction 평가
```

하지만 **W가 정말 공통인지는 가정일 뿐**이고, 다음이 필요함:
- CVD individual W 학습 후 HC common W와 직접 비교
- W matrix의 structure similarity 분석 (correlation, angle)
- Cross-subject generalization test

**만약 W가 다르다면:**
- Individual W 기반 필터 설계 필요
- Nonlinear transformation 고려 (linear Procrustes 불충분)
- Forward encoding model (color → neural response 예측) 개발

---

#### 2. Neural reconstruction ≠ Perceptual improvement

**현재 결과:**
- fMRI 기반 neural reconstruction에서 CVD가 HC-like 달성 (5° error)

**한계:**
Neural pattern이 HC처럼 변했다고 해서 **실제 색 지각이 개선되는지는 불명확.**

**필요한 검증:**
- Behavioral test: 필터 적용 전후 색 구분 능력 측정
- Subjective color naming: CVD가 HC와 유사하게 색 이름 부여하는지
- Discrimination threshold: JND (Just Noticeable Difference) 측정
- Perceptual uniformity: 색 간 지각적 거리 변화

**Neural-perceptual gap 해결 방안:**
- fMRI + psychophysics 동시 실험
- Decoding accuracy와 behavioral performance 상관관계 분석

---

#### 3. HC Common W가 Individual W보다 못한 이유 불명확

**현재 결과:**
```
HC Aligned (Common W): 5-15° error
HC Baseline (Individual W): 30-40° error
→ Common W가 더 좋음 (!)

HC No-align (Common W): 49-70° error
→ Alignment 없으면 오히려 baseline보다 나쁨
```

**한계:**
왜 no-alignment가 individual W보다 나쁜지 명확하지 않음.

**가능한 설명:**
1. **Voxel correspondence 문제**: Alignment 없이는 "같은 voxel"이 다른 색 정보 encoding
2. **Feature selection 차이**: Individual analysis에서 subject-specific voxel 선택됨
3. **Common W의 평균 효과**: 모든 subject에 최적화되지 않음 (trade-off)

**향후 필요:**
- Individual W vs Common W의 voxel weight 비교
- Alignment가 voxel-level correspondence를 어떻게 개선하는지 정량화
- Subject-specific feature importance 분석

---

#### 4. Linear transformation의 한계

**현재 방법:**
- Procrustes: Linear transformation (rotation + scaling + translation)

**한계:**
CVD-HC 간 transformation이 **nonlinear**일 가능성:
- Color space warping (특정 색 영역만 왜곡)
- Spatially heterogeneous transformation (voxel마다 다른 변환)

**향후 방향:**
- Nonlinear alignment (kernel methods, neural network-based)
- Hyperalignment (stimulus-specific transformation)
- Forward model로 direct mapping 학습
