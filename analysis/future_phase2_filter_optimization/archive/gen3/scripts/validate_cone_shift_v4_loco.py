#!/usr/bin/env python3
"""
validate_cone_shift_v4_loco.py — Validate ΔRDM-fitted cone-shift on V4 LOCO.

Target: V4 LOCO vulnerability (interpolation failure)
Method: W-fixed LOCO simulation, Spearman ρ, exact 8! permutation

Takes best δθ from fit_cone_shift_delta_rdm.py (fitted with ΔRDM loss),
validates it using V4 LOCO as the evaluation target:
  - ΔRDM-fitted δθ → HC W-fixed LOCO simulation → 8-color vulnerability
  - Compare with CVD observed LOCO (ridge_gcv from forward model)
  - Question: Does ΔRDM-fitted δθ generalize to LOCO criterion?

Validation logic:
  1. Load fit_cone_shift_delta_rdm result.json → best_δθ
  2. Simulate mean-HC LOCO vulnerability at best_δθ (W-fixed)
  3. Compare with CVD LOCO target (Spearman ρ)
  4. Exact 8! permutation: label_perm_p, baseline_improvement_p

Usage:
    conda activate srm
    python scripts/validate_cone_shift_v4_loco.py \
        --sim_dir results/sim \
