...
Handles both pilot (P01) and test (01-04) data structures

Usage:
    python build_roi_masks.py --subject P01  # For pilot
    python build_roi_masks.py --subject 01   # For test subject 01
"""
...
def find_reference_image(subject_id, fmriprep_base):
    ...
    if os.path.exists(func_dir):
        # Look for any preprocessed bold file (with or without res-2)
        patterns = [
            f"{func_dir}/sub-{subject_id}_task-*_run-*_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz",
        ]
    ...
    raise FileNotFoundError(
        ...
        f"  Patterns tried:\n"
        f"    - sub-{subject_id}_task-*_run-*_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz\n"
        f"    - sub-{subject_id}_task-*_run-*_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz"
    )
...
def find_brain_mask(subject_id, fmriprep_base):
    ...
    # Try anat directory first (with or without res-2)
    anat_dir = f"{fmriprep_base}/sub-{subject_id}/anat"
    if os.path.exists(anat_dir):
        patterns = [
            f"{anat_dir}/sub-{subject_id}_*_space-MNI152NLin2009cAsym_res-2_desc-brain_mask.nii.gz",
        ]
        for pattern in patterns:
            candidates = glob.glob(pattern)
            if candidates:
                print(f"  ✓ Found brain mask (anat): {os.path.basename(candidates[0])}")
                return candidates[0]
...
def find_reference_image_in_dir(fmriprep_dir):
    """Find a reference functional image for resampling - works with actual directory"""

    func_dir = f"{fmriprep_dir}/func"

    if not os.path.exists(func_dir):
        raise FileNotFoundError(f"func directory not found: {func_dir}")

    # List all files
    all_files = os.listdir(func_dir)

    # PRIORITY 1: Try to find res-2 files first (for pilot data at 97x115x97)
    for filename in all_files:
        if ("_task-" in filename and
            "space-MNI152NLin2009cAsym" in filename and
            "_res-2_" in filename and  # EXPLICITLY CHECK FOR res-2
            filename.endswith("desc-preproc_bold.nii.gz")):
            ref_path = os.path.join(func_dir, filename)
            print(f"  ✓ Found reference image (res-2): {filename}")

            # DEBUG: Load and check actual shape
            try:
                ref_img_debug = nib.load(ref_path)
                print(f"    [DEBUG] Reference image shape: {ref_img_debug.shape[:3]}")
                print(f"    [DEBUG] Reference image path: {ref_path}")
                print(f"    [DEBUG] Expected shape: (97, 115, 97) for res-2")
            except Exception as e:
                print(f"    [DEBUG] Could not load image for shape check: {e}")

            return ref_path

    raise FileNotFoundError(
        f"No reference functional image found\n"
        f"  Directory: {func_dir}\n"
        f"  Files found: {len(all_files)}\n"
        f"  First 3 files: {all_files[:3]}"
    )


def find_brain_mask_in_dir(fmriprep_dir):
    """Find brain mask for intersection (optional) - works with actual directory"""

    # Try anat directory first
    anat_dir = f"{fmriprep_dir}/anat"
    if os.path.exists(anat_dir):
        all_files = os.listdir(anat_dir)
        for filename in all_files:
            if ("space-MNI152NLin2009cAsym" in filename and
                filename.endswith("desc-brain_mask.nii.gz")):
                mask_path = os.path.join(anat_dir, filename)
                print(f"  ✓ Found brain mask (anat): {filename}")
                return mask_path
    ...
    if os.path.exists(func_dir):
        ...
        for filename in all_files:
            if ("space-MNI152NLin2009cAsym" in filename and
                filename.endswith("desc-brain_mask.nii.gz")):
                mask_path = os.path.join(func_dir, filename)
                print(f"  ✓ Found brain mask (func): {filename}")
                return mask_path
...
def build_roi_mask(roi_name, roi_parts, project_dir, ref_img, brain_mask_path, output_path):
    ...
    # DEBUG: Print atlas space info
    print(f"    [DEBUG] Atlas space - Shape: {roi_mask.shape}, Voxels: {np.sum(roi_mask)}")

    # Convert to NIfTI
    roi_nii = nib.Nifti1Image(roi_mask.astype(np.int16), part_img.affine)

    # DEBUG: Print target resampling info
    print(f"    [DEBUG] Target functional space - Shape: {ref_img.shape[:3]}")
    ...
    # DEBUG: Print after resampling (before brain mask)
    voxels_after_resample = int(np.sum(roi_resampled.get_fdata() > 0))
    print(f"    [DEBUG] After resampling - Shape: {roi_resampled.shape[:3]}, Voxels: {voxels_after_resample}")
    ...
    if brain_mask_path and os.path.exists(brain_mask_path):
        try:
            ...
            print(f"    [DEBUG] Brain mask shape before resample: {brain_img.shape[:3]}")
            ...
            voxels_before_mask = int(np.sum(roi_bool))
            voxels_after_mask = int(np.sum(combined))
            print(f"    [DEBUG] Brain mask intersection: {voxels_before_mask} -> {voxels_after_mask} voxels")
...
def main():
    ...
    file_prefix = subject_id
    
    # Handle pilot data: P01 = pilot (in sub-P01/), 01-04 = test subjects
    if subject_id.startswith("P"):
        file_prefix = subject_id[1:]
    ...
    print(f"  [INFO] Test subject ({subject_id})")
    ...
    try:
        # Use the actual fmriprep_dir we determined above
        ref_img_path = find_reference_image_in_dir(fmriprep_dir)
    ...
    brain_mask_path = find_brain_mask_in_dir(fmriprep_dir)
    print()

    # Create output directory (use sub-P01 for pilot to distinguish from test sub-01)
    ...
    for roi_name, roi_parts in WANG_ROI_MAP.items():
        output_path = os.path.join(output_dir, f"sub-{file_prefix}_{roi_name}_mask.nii.gz")
