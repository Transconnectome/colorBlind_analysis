...
-----------------------
FIR test with FEATURE SELECTION + REGULARIZATION (Option A)

Addresses overfitting by:
1. Selecting only top 30 most informative voxels (SelectKBest)
2. Strong L2 regularization (C=0.01)

This reduces effective parameters from 2,177 to ~203
(30 features × 7 classes + 7 biases)
Still much more than 40 samples, but more reasonable.
"""
...
print("="*70)
print("FIR Test with Feature Selection + Regularization (Option A)")
print("="*70)
print(f"Subject: sub-{cfg.SUB_ID}")
print()
print("Overfitting mitigation:")
print("  1. SelectKBest: Top 30 most discriminative voxels")
print("  2. Strong L2 regularization: C=0.01")
print("  3. Expected parameters: ~203 (vs 40 samples)")
print()
sys.stdout.flush()
...
K_FEATURES = 30  # Number of voxels to select
...
# ============================================================================
# Classification with Feature Selection + Regularization
# ============================================================================

print(f"[5/5] Classification with feature selection + regularization")
print(f"  Feature selection: SelectKBest (k={K_FEATURES})")
print(f"  Regularization: LogisticRegression(C=0.01, penalty='l2')")
sys.stdout.flush()
...
for test_run in range(N_RUNS):
    ...
    # Train classifier with strong regularization
...
effective_params = K_FEATURES * (N_COLORS - 1) + (N_COLORS - 1)
train_samples = len(train_runs) * N_COLORS
print(f"Effective parameters: {effective_params}")
print(f"Training samples: {train_samples}")
print(f"Samples per parameter: {train_samples/effective_params:.3f}")
print()
sys.stdout.flush()
