...
To evaluate color display filter conditions, CVD participants underwent two sessions. While the first session utilized original stimuli in all runs, the second session consisted of two runs for each of three conditions: colors modified by the Windows filter, by the personalized filter (parameters described in §\textit{Filter design} below), and original stimuli. CVD neural responses under each filter condition were compared against HC responses to original stimuli.
...
\textbf{Leave-one-run-out (LORO)} tested whether individual colors could be reliably distinguished from voxel responses, assessing discrimination consistency across stimulus repetitions. For each of six runs, the corresponding response matrices ($B$, $C$) were held out as the test set, $W$ was trained on the remaining five runs, and voxel responses were predicted (Equation 6b).

\textbf{Leave-one-color-out (LOCO)} tested whether the model could interpolate to novel hues not seen during training, assessing the preservation of continuous color-space geometry. For each color, all six run response matrices of the target color were held out. $W$ was trained on the remaining seven colors, and voxel responses to the held-out color were predicted (Equation 6b).

Both were evaluated using Pearson correlation between predicted and observed response vectors, computed across all voxels and test samples. Significance of LOCO performance was assessed by comparing the observed Pearson $r$ against a null distribution obtained from 1,000 random color-label permutations. LORO performance indicates color discriminability, while LOCO performance reflects the intactness of continuous hue-space geometry.

\section{Filter design}
\label{sec:methods:filter}

CVD color distortion was modeled as a two-component hue dilation in CIELab opponent-color space (Equation~\ref{eq:2comp}):
...
where $h$ is the opponent-hue angle of the stimulus, derived from CIELab coordinates using the Machado simulation framework at zero spectral shift \cite{machado2009}; and $\theta_{\rm conf}$ is the CVD-subtype confusion axis in CIELab $a^*$-$b^*$ space (protan: $16^\circ$; deutan: $150^\circ$), obtained by projecting each subtype's missing-cone axis in LMS space \cite{brettel1997} through the Stockman--Sharpe cone fundamental matrix \cite{stockman2000} onto the $a^*$-$b^*$ plane at $L^* = 75$. $\beta_s$ weights distortion along the S-cone opponent axis ($90^\circ$), motivated by behavioral evidence that CVD perceptual errors concentrate near S-cone confusion loci \cite{emery2021}. $\beta_c$ weights distortion along the CVD-type confusion axis, capturing partial cortical compensation documented in V2/V3 \cite{tregillus2021}. The perceived hue was $\theta' = (\theta + \delta\theta) \bmod 360^\circ$. Parameters were estimated by a grid search over $\beta_s \in [0^\circ, 50^\circ]$ and $\beta_c \in [-50^\circ, 50^\circ]$ (26 $\times$ 51 = 1,326 combinations, $2^\circ$ resolution). The pair maximizing hV4 LOCO Pearson $r$ was retained. Permutation significance testing followed the same procedure as for the encoding model.

The personalized stimulus-space filter was derived as the inverse of the fitted transformation $T(\hat{\beta}_s, \hat{\beta}_c)$. For each stimulus hue $\theta_k$, the pre-image $\tilde{\theta}_k$ satisfies $T(\tilde{\theta}_k) = \theta_k$. A CVD observer viewing $\tilde{\theta}_k$ thus perceives the same cortical hue as a healthy observer viewing $\theta_k$. Pre-images were computed numerically by dense-grid initialization (1,440 candidate values) followed by Brent's method refinement at the sign-change bracket. Numerical accuracy was verified by confirming $|T(\tilde{\theta}_k) - \theta_k| < 0.001^\circ$ for all eight hues and both CVD participants.

\section{Behavioral-neural concordance}

\subsection*{Behavioral tasks}

Participants completed two behavioral tasks under display settings matching the fMRI experiment.
