...
DERIVATIVES_DIR = BASE_DIR / 'derivatives' / 'V3_Comprehensive' / 'ROI_mask'
...
class ROIPipelineComprehensive:
    ...
    def __init__(self, subject_id, run_id=1, dataset='original_v3'):
        # Standardize subject_id to '01', '02', etc. format
        ...
        self.subject_id = str(subject_id).zfill(2)
        self.run_id = run_id
        self.dataset = dataset

        # Use dataset-specific fMRIPrep directory
        self.fmriprep_dir = Path(DATASET_PATHS[dataset])
    ...
    def setup_paths(self):
        ...
        # Subject directories (original_v3: all subjects in main directory, no pilot)
        ...
        # Use simpler naming without timestamp for easier access
        self.output_dir = self.deriv_subj_dir / 'roi_pipeline'
    ...
    def _get_anat_ref(self):
        ...
        pattern = f'sub-{self.subject_id}_*space-{TARGET_SPACE}_res-{TARGET_RES}_*desc-preproc_T1w.nii.gz'
    ...
    def _get_func_brain_mask(self):
        ...
        pattern = f'sub-{self.subject_id}_task-rsvp_run-{self.run_id}_space-{TARGET_SPACE}_res-{TARGET_RES}_desc-brain_mask.nii.gz'
    ...
    def _get_gm_probseg(self):
        ...
        pattern = f'sub-{self.subject_id}_*probseg*'
...
def main():
    """Main entry point"""
    # Arguments are already parsed at the top of the file (args)
    # Run pipeline with dataset specification
    pipeline = ROIPipelineComprehensive(args.subject_id, args.run, args.dataset)
