...
## 부록 D: SRM vs Procrustes — MDS/Mantel 진단

### D.1 동기 및 연구 질문

Phase 1 forward model의 MDS 분석에서 V1/V2는 SRM 투영 하에 4가지 진단 기준 모두 실패했다. 이에 두 가설을 검정한다:

- **H1 (참조 모델 문제)**: 등간격 angular RDM이 부적절하며, 지각적 CIELab 거리를 사용하면 구조가 복원될 것이다.
- **H2 (진정한 구조 부재)**: SRM 차원 축소(99%+ 압축: 예, 568 복셀 → 3차원)가 연속적 색 공간 기하를 비가역적으로 파괴한다.

본 부록은 3가지 정렬 조건(raw, Procrustes, SRM)에서 4개 ROI의 신경 RDM을 angular 및 CIELab 참조 모델과 비교하여 H1과 H2를 검정한다.

### D.2 방법

**데이터**: C010 + Procrustes, 10명 (HC 7명, CVD 3명), 8색 (45° 등간격 색상각).

**정렬 조건**: (1) Raw 복셀 공간, (2) Procrustes 정렬 복셀 공간, (3) SRM (k = V1:4, V2:4, V3:3, hV4:3).

**집단 평균 RDM**: HC 피험자 개인별 8×8 상관 거리 RDM의 평균.

#### Mantel 검정 (상세 서술)

Mantel 검정(Mantel, 1967)은 두 거리(비유사도) 행렬 간 연관성을 검정하는 비모수 순열 검정이다.

**절차**:
1. 신경 RDM과 참조 RDM의 상삼각 원소(8×8에서 28개 고유 쌍) 추출.
2. 두 벡터 간 Spearman 순위 상관(r_obs) 계산.
3. 귀무 분포 생성: 신경 RDM의 행과 열을 동시에 10,000회 순열, 매번 r 재계산.
4. 단측 p-value: p = (count(r_perm ≥ r_obs) + 1) / (n_perm + 1).

**해석**:
- r > 0: 신경 거리가 참조 구조를 반영 (구조 존재).
- r ≈ 0: 연관 없음.
- r < 0: **역상관** — 투영이 거리 관계를 반전시킴.

**부호 반전의 의미**: 동일 신경 데이터에서 raw r = +0.402이나 SRM r = −0.308인 경우, SRM의 극단적 차원 축소(예: hV4 568복셀 → 3차원)가 색 간 거리 구조를 비가역적으로 파괴하고 반전시켰음을 의미한다.

**4가지 참조 모델**:
1. **Angular** (등간격): min(|θ_i − θ_j|, 360° − |θ_i − θ_j|) / 180°
2. **CIELab a\*b\***: 지각적 a\*,b\* 평면 유클리드 거리 (정규화)
3. **a\*-only**: L−M cone-opponent 축 (1D)
4. **b\*-only**: S−(L+M) cone-opponent 축 (1D)

**다중 비교 보정**: Bonferroni α = 0.05 / (4 ROI × 4 참조) = 0.003125.

#### 6개 분석 모듈

1. **Stress 곡선 (1–7D)**: 각 차원에서 metric MDS의 Kruskal 정규화 stress-1.
2. **참조 RDM 비교 (Mantel)**: 4개 참조 모델 대비 신경 RDM, 모든 정렬 조건.
3. **Persistent Homology (H1)**: Rips filtration으로 1차원 루프 탐지; max H1 lifetime의 순열 검정.
4. **고차원 MDS + PCA**: 2D/3D/4D MDS 후 PCA로 최적 2D 평면 추출. 지표: stress, Shepard R², 원형 순위 상관(ρ).
5. **Isomap vs MDS**: 비선형(Isomap, n_neighbors=3) vs 선형(MDS) 2D 임베딩. 다양체 곡률 검정.
6. **피험자별 분석**: 개인별 지표(stress, circular ρ, ISC, Mantel r).

**판정 기준**: 4개 중 ≥2개 통과(Q1: CIELab > Angular; Q2: H1 유의; Q3: 고차원에서 stress < 0.10 또는 ρ > 0.7; Q4: Isomap > MDS) → STRUCTURED.

### D.3 결과

#### D.3.1 CIELab 비균등성

등간격 색상각(45° 간격) 8색의 인접 CIELab 거리:
...
범위: 30.9–68.3 (비율 = 2.2배). 각도 공간에서 등간격인 색상이 지각적 CIELab 공간에서는 상당히 불균등하다.

#### D.3.2 Stress 곡선 (1–7D)

HC 집단 평균 RDM의 Kruskal 정규화 stress-1:

| 차원 | V1 raw | V1 Proc | V1 SRM | V2 raw | V2 Proc | V2 SRM | V3 raw | V3 Proc | V3 SRM | hV4 raw | hV4 Proc | hV4 SRM |
|-----:|-------:|--------:|-------:|-------:|--------:|-------:|-------:|--------:|-------:|--------:|---------:|--------:|
...
굵은 글씨 = stress < 0.10 달성 최초 차원.

**핵심 관찰**:
- **SRM 고원 현상**: 모든 ROI에서 dim ≥ k (SRM 차원)부터 stress가 고원에 도달, 이후 감소 없음. SRM 투영이 경성 정보 병목임을 확인.
- **V1 SRM**: stress < 0.10에 도달하지 못함 (고원: 0.126–0.128). 모든 거리 구조 소실.
- **hV4 SRM**: 2D에서 0.084로 stress < 0.10 달성, 3D부터 0.063에서 고원. 모든 ROI 중 hV4 SRM이 최저 stress — 그러나 Mantel 검정은 여전히 실패 (아래 참조).
- **Raw/Procrustes**: 4–5D에서 stress < 0.10 달성, 7D까지 계속 감소. 인위적 고원 없음.

#### D.3.3 Mantel 검정 (전체 48개 검정)

HC 집단 평균 신경 RDM vs 4개 참조 모델, 3개 정렬 조건, 4개 ROI:

| ROI | 정렬 | Angular r (p) | CIELab r (p) | a\*-only r (p) | b\*-only r (p) |
|-----|------|:-------------:|:------------:|:--------------:|:--------------:|
...
\* p < 0.05 (비보정); Bonferroni 보정(α = 0.003125) 후 유의한 것은 없음.

**핵심 발견**: hV4 raw CIELab r = +0.402 (p = 0.018) → SRM r = −0.308 (p = 0.966). **완전한 부호 반전**. Raw 복셀 공간에서 유의한 지각적 구조를 보이는 동일 신경 데이터가, SRM k=3 투영(568복셀 → 3차원 = 99.5% 압축)에 의해 구조가 파괴되고 반전됨.

**V2 a\*-only 경향**: V2는 모든 정렬에서 일관된 L−M 축 경향 (Proc r = 0.302, p = 0.071; SRM r = 0.282, p = 0.085). SRM 고유 현상이 아님 — Procrustes에서도 동일 패턴으로, V2의 진정한 L−M cone-opponent 신호가 차원 축소 하에서도 부분적으로 보존됨을 시사.

#### D.3.4 Persistent Homology (H1)

| ROI | Max H1 Lifetime | p-value | 결과 |
|-----|----------------:|--------:|------|
| V1 | 0.448 | 1.000 | FAIL |
| V2 | 0.156 | 1.000 | FAIL |
| hV4 | 0.279 | 1.000 | FAIL |

SRM 공간에서 유의한 1차원 루프(원형 위상)를 보이는 ROI 없음. 모든 H1 lifetime이 순열 귀무분포 내에 위치.

#### D.3.5 고차원 MDS (SRM 공간)

| ROI | 차원 | Stress | Shepard R² | Circular ρ | p |
|-----|-----:|-------:|-----------:|-----------:|-----:|
...
**핵심**: hV4가 최저 stress(3D 0.063)와 최고 Shepard R²(0.964)를 달성하여, SRM 3D 공간 내 거리 보존은 양호. 그러나 원형 순서 상관은 모든 차원에서 비유의 — 거리는 보존되나 **원형 색상 순서**는 보존되지 않음. 이 해리(낮은 stress + 낮은 circular ρ)는 SRM이 피험자 내 색 기하가 아닌 피험자 간 공유 분산 성분을 보존하는 특성의 징후.

#### D.3.6 Isomap vs MDS (SRM 공간)

| ROI | MDS ρ | MDS p | Isomap ρ | Isomap p | 우세 |
|-----|------:|------:|---------:|---------:|------|
| V1 | +0.619 | 0.102 | −0.476 | 0.233 | MDS |
| V2 | −0.262 | 0.531 | +0.524 | 0.183 | Isomap |
| hV4 | +0.452 | 0.260 | +0.048 | 0.911 | MDS |

V2만 Isomap이 MDS보다 우수 → L−M cone-opponent 축의 곡면 다양체 구조와 일치. hV4: MDS 우세(ρ = 0.452 vs 0.048) → SRM 공간의 잔여 구조가 다양체 곡률이 아닌 저차원 선형 구조.

#### D.3.7 피험자별 분석 (SRM 공간)

**V1** (10명):

| 피험자 | 그룹 | Stress | Circ. ρ | ISC | Angular r | CIELab r |
|--------|------|-------:|--------:|----:|----------:|---------:|
...
**주목할 점**: hV4 피험자별 ISC가 매우 가변적(0.096–0.816). sub-04 ISC = 0.103으로 극단적 이상치. sub-07 hV4는 16복셀만 보유 → SRM 공간에서 SNR 저하.

#### D.3.8 판정 프레임워크 (hV4 업데이트)

| ROI | Q1: CIELab > Angular | Q2: H1 위상 | Q3: 고차원 | Q4: Isomap > MDS | 통과 | 판정 |
|-----|:---:|:---:|:---:|:---:|:---:|---------|
...
**hV4 상세**: Q3 통과 — 3D stress = 0.063 < 0.10 (양호한 거리 적합). 그러나 Q1 실패(SRM CIELab r = −0.308, SRM angular r = −0.302, 양쪽 강한 음수); Q2 실패(H1 루프 없음); Q4 실패(MDS ρ = 0.452 >> Isomap ρ = 0.048). MARGINAL 판정은 SRM이 hV4에서 *일부* 거리 구조를 보존하나(전 ROI 중 최저 stress), 보간에 필요한 *색 특이적* 원형 기하는 보존하지 못함을 반영.

### D.4 해석

**H2 확인**: CIELab 참조도 실패 → 문제는 참조 모델이 아니라 SRM 투영 공간에서의 진정한 구조 부재. 4개 ROI 전체에서 일관.

**hV4 해리**: Raw 복셀 공간은 CIELab 구조를 보유(r = +0.402, p = 0.018*), 그러나 SRM이 이를 파괴(r = −0.308, p = 0.966). 이 **부호 반전**은 Phase 1 forward model에서 관찰된 +3° LOCO 페널티의 근본 원인을 직접 설명: SRM의 피험자 간 정렬이 공유 분산을 최적화하나, 이것이 반드시 색 공간 기하에 대응하지 않음. SRM 투영 RDM이 진정한 지각적 구조 대비 반전되면, 해당 투영 기반 forward model 보간은 체계적으로 오류 방향을 예측.

**V2 L−M 다양체**: 일관된 a\*-only 경향(Proc r = 0.302, SRM r = 0.282)과 Isomap 우세는 V2가 L−M cone-opponent 축 지배적인 곡면 3D 다양체를 보유함을 시사. 알려진 V2 thin-stripe 색 선택성과 일치.

**V1 음성 대조군**: 0/4 기준 통과. V1은 어떤 정렬 조건에서도 복원 가능한 색 구조를 보이지 않음. Phase 1의 "V1은 변별하나 보간 불가" 발견과 일관.

### D.5 Phase 2 필터 설계 시사점

| 기준 | Procrustes | SRM |
|------|:---------:|:---:|
| 거리 보존 | 양호 (7D까지 stress 감소) | k에서 고원 (경성 병목) |
| Mantel: hV4 CIELab | r = −0.191 (정렬로 파괴) | r = −0.308 (파괴 + 반전) |
| Mantel: V2 a\*-only | r = +0.302 (p = 0.071) | r = +0.282 (p = 0.085) |
| 원형 위상 (H1) | 미검정 (고차원) | 모든 ROI에서 부재 |
| 차원 유연성 | 전체 복셀 공간 | k에 고정 |

**결론**: Procrustes 정렬이 SRM보다 원래 거리 구조를 더 많이 보존. Phase 2 필터 최적화에서 예측 엔진은 SRM 공간이 아닌 Procrustes 정렬 복셀 공간(ridge_gcv + Procrustes)에서 작동해야 함. SRM은 Phase 2 RDM 기반 기준(저차원 비교)에는 여전히 유용하나, LORO/LOCO 기준은 Procrustes를 사용해야 함.

### D.6 그림 범례

- **fig1\_stress\_curve.png**: 4개 ROI × 3개 정렬의 MDS 차원(1–7D)별 Kruskal 정규화 stress-1. 회색 점선 = stress = 0.10 임계값; 주황 점선 = SRM k값. SRM 곡선은 k에서 고원.
- **fig2\_mantel\_comparison.png**: SRM 정렬의 Mantel 검정 귀무분포(10,000 순열). 4 ROI(행) × 4 참조 모델(열). 빨간 수직선 = r_obs (p < Bonferroni); 주황 = p < 0.05 비보정; 회색 = n.s.
- **fig3\_persistence.png**: V1, V2, hV4의 SRM 공간 H1 persistence diagram(좌: birth-death, 우: 순열 귀무).
- **fig4\_higher\_d\_mds.png**: V1, V2, hV4의 2D/3D→PCA/4D→PCA MDS 임베딩. 색 코딩 점 + stress, Shepard R², circular ρ 주석.
- **fig5\_isomap\_vs\_mds.png**: V1, V2, hV4의 SRM 공간 MDS vs Isomap 2D 임베딩 비교.
- **fig6\_per\_subject.png**: V1, V2, hV4 피험자별 지표(stress, circular ρ, ISC, CIELab Mantel r). 파란 원 = HC; 색 다이아몬드 = CVD (빨강 = sub-08, 주황 = sub-09, 보라 = sub-10).
