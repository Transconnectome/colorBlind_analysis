...
# 5. Circular Color Space Visualization (original simple version)
fig = plt.figure(figsize=(12, 5))

# Left: Training colors reconstruction
ax1 = fig.add_subplot(1, 2, 1, projection='polar')
for result in reconstruction_results:
    true_hues_rad = np.deg2rad(result['true_hues'])
    recon_hues_rad = np.deg2rad(result['reconstructed_hues'])

    ax1.scatter(true_hues_rad, np.ones(len(true_hues_rad)), s=150, alpha=0.3, c='blue', label='True' if result['test_run']==1 else '')
    ax1.scatter(recon_hues_rad, np.ones(len(recon_hues_rad))*0.9, s=100, alpha=0.5, c='red', marker='x', label='Reconstructed' if result['test_run']==1 else '')
...
for result in novel_color_results:
    ...
    # Get all reconstructed values for this color (from all runs)
    recon_values = []
    ...
    ax2.scatter(np.deg2rad(true_hue), 1, s=150, alpha=0.6, c='blue')
    ax2.scatter(np.deg2rad(mean_recon), 0.9, s=100, alpha=0.8, c='red', marker='x')

    # Draw error arc
    error = result['mean_error']
    arc_angles = np.linspace(np.deg2rad(true_hue), np.deg2rad(mean_recon), 20)
    ax2.plot(arc_angles, np.ones(20)*0.95, 'gray', alpha=0.3, linewidth=1)

ax2.set_ylim([0, 1.1])
ax2.set_title(f'Novel Colors (Leave-One-Out)\nMean error: {mean_novel_error:.1f}°\nChance: 90.0°')

plt.suptitle(f'{ROI_NAME}: Circular Color Space', fontsize=14, fontweight='bold')
plt.tight_layout()
