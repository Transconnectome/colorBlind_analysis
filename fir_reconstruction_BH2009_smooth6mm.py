...
----------------------------------
BH2009 pipeline with 6mm smoothing (Config 14/18 from grid search)

PREPROCESSING (Config 14/18 - Good balance):
- Spatial smoothing: 6mm FWHM Gaussian kernel (BETTER SPATIAL SPECIFICITY)
...
GRID SEARCH RESULTS:
Config 14/18 (6mm smoothing) achieved:
- HRF variability: 0.9971 (excellent, only slightly lower than 8mm's 0.9998)
- Temporal SNR: 59.5 (good, vs 8mm's 89.5)
- Better spatial specificity: Less signal leakage between ROIs
- Preserves fine-grained spatial patterns better

TRADE-OFF ANALYSIS (6mm vs 8mm):
6mm Advantages:
  - Better spatial resolution (3-voxel blur vs 4-voxel blur)
  - Less signal leakage from adjacent ROIs
  - Preserves multivariate patterns better
  - Standard in fMRI literature

8mm Advantages:
  - Slightly higher HRF homogeneity (0.9998 vs 0.9971)
  - Higher temporal SNR (89.5 vs 59.5)
  - Better noise reduction

RECOMMENDATION: Try both and compare decoding performance!

Usage:
    python fir_reconstruction_BH2009_smooth6mm.py --roi V1 --subject P01

Created: 2025-01-22
Based on: Grid search Config 14/18 (6mm smoothing + motion_6)
"""
...
# Config 14/18 Preprocessing Parameters (6mm smoothing for better spatial specificity)
SMOOTHING_FWHM = 6  # mm - From grid search Config 14/18 (better spatial resolution)
...
def parse_args():
    parser = argparse.ArgumentParser(description='B&H 2009 with 6mm smoothing (better spatial specificity)')
    ...
    parser.add_argument('--output-dir', type=str, default=None,
                        help='Output directory (default: derivatives/BH2009_smooth6mm/timestamp/)')
...
if args.output_dir:
    ...
    if SUBJECT_ID == 'P01':
        output_dir = Path(f"derivatives/BH2009_smooth6mm/pilot/{timestamp}_{DERIVATIVE_PREFIX}_{ROI_NAME}")
    else:
        output_dir = Path(f"derivatives/BH2009_smooth6mm/{timestamp}_{DERIVATIVE_PREFIX}_{ROI_NAME}")
...
print("="*70)
print("B&H (2009) Pipeline with 6mm Smoothing (Better Spatial Specificity)")
print("="*70)
print(f"Subject: {SUBJECT_ID}")
print(f"ROI: {ROI_NAME}")
print()
print("Config 14/18 Preprocessing (6mm smoothing):")
print(f"  - Spatial smoothing: {SMOOTHING_FWHM}mm FWHM (better spatial resolution)")
...
print(f"[2/9] Loading functional data with 6mm smoothing preprocessing ({N_RUNS} runs)")
sys.stdout.flush()
...
if len(hrf_correlations) > 0:
    ...
    if np.mean(hrf_correlations) > 0.85:
        print(f"  ✅ High HRF homogeneity (mean r = {np.mean(hrf_correlations):.3f})")
        print(f"     → Config 26 preprocessing successfully improved HRF consistency!")
    else:
        print(f"  ⚠️  Moderate HRF homogeneity (mean r = {np.mean(hrf_correlations):.3f})")
...
results_summary = {
    'subject': SUBJECT_ID,
    'roi': ROI_NAME,
    'preprocessing_config': 'Config 14/18 (6mm smoothing)',
...
if np.mean(hrf_correlations) > 0.85:
    print("✅ 6mm smoothing achieved high HRF homogeneity!")
    print("   Better spatial specificity while maintaining voxel consistency.")
else:
