...
#### Permutation Test (10K, HC ridge_gcv, FE-6)
...
V1/V2 null ~0.10-0.13 (not zero) — voxel covariance 구조가 baseline voxel_corr 생성. **hV4만 covariance baseline 초과.**
...
#### HC 결과 — 3-tier 비교 (voxel_corr)

| ROI | ZS (직접) | LORO (ridge_gcv) | LOCO (ridge_gcv) | t(ZS-LORO) | p |
|-----|:---------:|:----------------:|:----------------:|:----------:|:---:|
...
1. **hV4만 ZS ≈ LORO** (p=0.913): Group prior만으로도 subject-specific ridge_gcv와 동등한 패턴 재현 → **hV4의 group prior가 Phase 2 filter의 신뢰할 수 있는 prediction engine**
2. **V1/V2/V3: ZS >> LORO** (p<0.003): ZS가 6-run 평균과 비교 vs LORO는 single run → noise 차이. Group prior가 spatial pattern 재현은 하지만, V1/V2에서는 interpolation 불가 (LOCO FAIL 유지)
3. **LOCO 항상 최저**: 보간(interpolation)이 가장 어려운 과제 — run generalization (LORO)보다, 공간 패턴 재현 (ZS)보다 훨씬 어려움
...
**HC ≈ CVD** (전 ROI p>0.4). ZS 직접 평가는 공간 패턴 재현 능력 → HC-CVD 구별 불가. **LOCO가 유일한 HC-CVD 해리 도구** (보간 정확도만이 차이를 포착).
...
**해석**: 양수 gap = HC가 색상 간 보간 우수. CVD의 왜곡된 hue geometry 반영 (신호 부재 아님 — CVD LORO ≈ HC). Gap 크기는 모델 명세에 따라 변동 (§8c 참조).
...
### 8d. Per-Color Residual — Cone Shift 일관성
...
> Warm (red–green): FE-3에서 HC-CVD gap 없음 (전 |d| < 1, 전 p > 0.2).
> Cool (blue, purple): d > 1.3, trending 또는 유의.
...
SRM으로 39-56% 피험자간 분류 정확도. 우리의 10/12 CVD→HC cross-decoding 성공과 일치. CVD의 결함은 hue-space 변환(T_psi), voxel-space 재조직이 아님. Voxel preference 결과 확인: green 부족, magenta 과잉, 그러나 **동일 복셀** — argmax만 이동.
