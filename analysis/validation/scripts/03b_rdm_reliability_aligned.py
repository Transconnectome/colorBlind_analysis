#!/usr/bin/env python3
"""
3.1.1b Across-run RDM Reliability (After Procrustes Alignment)
==============================================================
Procrustes alignment 후 run 간 RDM 일관성 검증

Procrustes Alignment:
- 각 run을 reference run에 정렬
- Rotation + reflection 허용 (scaling 없음)
- 신경 표현의 기하학적 구조 보존하면서 정렬

비교:
- Before alignment: 원본 amplitudes로 RDM 계산
- After alignment: Procrustes-aligned amplitudes로 RDM 계산
- Expected: alignment 후 reliability 향상

Input:
- Baseline amplitudes: {BASELINE_RESULTS}/sub-{ID}/{ROI}/amplitudes_z.npy
  - Shape: (n_runs=6, n_colors=8, n_voxels)

Output:
- {VALIDATION_OUT}/rdm_reliability_aligned/{TIMESTAMP}/
  - within_subject_reliability_aligned.json
  - rdm_reliability_comparison.png (before vs after)
  - alignment_disparities.png (run-pair alignment quality)
"""

import argparse
import json
import sys
from pathlib import Path
from datetime import datetime
import numpy as np
import matplotlib.pyplot as plt
from scipy.stats import spearmanr
from scipy.spatial import procrustes
from itertools import combinations
from tqdm import tqdm

plt.rcParams['figure.dpi'] = 150
plt.style.use('default')


def compute_rdm(amplitudes):
    """
    Compute RDM (Representational Dissimilarity Matrix)

    Args:
        amplitudes: (n_colors, n_voxels) array

    Returns:
        rdm: (n_colors, n_colors) dissimilarity matrix
    """
    # Correlation matrix between colors
    corr_matrix = np.corrcoef(amplitudes)

    # Dissimilarity = 1 - correlation
    rdm = 1 - corr_matrix

    return rdm


def remove_constant_voxels(amplitudes_z):
    """
    Remove voxels that have zero variance across colors in any run.

    Args:
        amplitudes_z: (n_runs, n_colors, n_voxels)

    Returns:
        cleaned_amplitudes: (n_runs, n_colors, n_valid_voxels)
        n_removed: number of voxels removed
        valid_mask: boolean mask of valid voxels
    """
    n_runs, n_colors, n_voxels = amplitudes_z.shape

    # Track voxels that are constant in any run
    invalid_voxels = set()

    for run_idx in range(n_runs):
        pattern = amplitudes_z[run_idx]  # (n_colors, n_voxels)

        # Compute std across colors for each voxel
        voxel_stds = np.std(pattern, axis=0)  # (n_voxels,)

        # Mark voxels with zero variance
        constant_voxels = np.where(voxel_stds < 1e-10)[0]
        invalid_voxels.update(constant_voxels)

    # Create valid voxel mask
    valid_mask = np.ones(n_voxels, dtype=bool)
    if len(invalid_voxels) > 0:
        valid_mask[list(invalid_voxels)] = False

    n_valid = np.sum(valid_mask)
    n_removed = len(invalid_voxels)

    # Filter amplitudes
    cleaned_amplitudes = amplitudes_z[:, :, valid_mask]

    return cleaned_amplitudes, n_removed, valid_mask


def procrustes_align_runs(amplitudes_z, reference_run=0):
    """
    Procrustes alignment of all runs to a reference run

    Args:
        amplitudes_z: (n_runs, n_colors, n_voxels)
        reference_run: Index of reference run (default: 0)

    Returns:
        aligned_amplitudes: (n_runs, n_colors, n_voxels) aligned data
        disparities: (n_runs,) disparity values (0 for reference)

    Raises:
        ValueError: If data has zero variance, contains NaN/Inf, or lacks color discriminability
    """
    n_runs, n_colors, n_voxels = amplitudes_z.shape

    # Check for NaN/Inf
    if np.isnan(amplitudes_z).any() or np.isinf(amplitudes_z).any():
        raise ValueError("Input data contains NaN or Inf values")

    # Remove constant voxels (zero std across colors in any run)
    cleaned_amplitudes, n_removed, valid_mask = remove_constant_voxels(amplitudes_z)
    n_valid_voxels = cleaned_amplitudes.shape[2]

    if n_removed > 0:
        print(f"    Removed {n_removed} constant voxels, {n_valid_voxels} remain")

    # Check if enough voxels remain
    if n_valid_voxels < 5:
        raise ValueError(f"Too few valid voxels ({n_valid_voxels}) after removing constant voxels. "
                       f"Removed {n_removed}/{n_voxels} constant voxels.")

    # Use cleaned data for alignment
    amplitudes_z = cleaned_amplitudes

    # Check for zero variance (overall)
    for run_idx in range(n_runs):
        var = np.var(amplitudes_z[run_idx])
        if var < 1e-10:
            raise ValueError(f"Run {run_idx} has zero or near-zero variance (var={var:.2e})")

    # Check for color discriminability (unique color patterns)
    # Procrustes requires >1 unique points (i.e., colors must be distinguishable)
    for run_idx in range(n_runs):
        pattern = amplitudes_z[run_idx]  # (n_colors, n_voxels)

        # Compute pairwise distances between colors
        from scipy.spatial.distance import pdist
        distances = pdist(pattern)
        min_distance = np.min(distances)

        # If all colors are too similar, Procrustes will fail
        if min_distance < 1e-6:
            raise ValueError(f"Run {run_idx} has no color discriminability (min distance={min_distance:.2e}). "
                           f"All colors have nearly identical patterns (no task response).")

    # Reference pattern
    reference = amplitudes_z[reference_run]  # (n_colors, n_voxels)

    aligned_amplitudes = np.zeros_like(amplitudes_z)
    disparities = np.zeros(n_runs)

    # Reference run stays the same
    aligned_amplitudes[reference_run] = reference
    disparities[reference_run] = 0.0

    # Align other runs to reference
    for run_idx in range(n_runs):
        if run_idx == reference_run:
            continue

        pattern = amplitudes_z[run_idx]  # (n_colors, n_voxels)

        try:
            # Procrustes alignment
            mtx1, mtx2, disparity = procrustes(reference, pattern)
            aligned_amplitudes[run_idx] = mtx2
            disparities[run_idx] = disparity
        except ValueError as e:
            # Procrustes failed (likely duplicate points)
            raise ValueError(f"Procrustes alignment failed for run {run_idx}: {e}")

    return aligned_amplitudes, disparities


def compute_rdm_reliability(amplitudes_z, use_alignment=False):
    """
    Compute run-pair RDM correlation (reliability)

    Args:
        amplitudes_z: (n_runs, n_colors, n_voxels)
        use_alignment: Whether to apply Procrustes alignment first

    Returns:
        dict with:
            - pairwise_correlations: list of r values for all run pairs
            - mean_correlation: mean reliability
            - run_rdms: list of RDM matrices
            - disparities: alignment disparities (if use_alignment=True)
    """
    n_runs, n_colors, n_voxels = amplitudes_z.shape

    # Optional: Procrustes alignment
    if use_alignment:
        amplitudes_aligned, disparities = procrustes_align_runs(amplitudes_z)
    else:
        amplitudes_aligned = amplitudes_z
        disparities = None

    # Compute RDM for each run
    run_rdms = []
    for run_idx in range(n_runs):
        rdm = compute_rdm(amplitudes_aligned[run_idx])  # (n_colors, n_colors)
        run_rdms.append(rdm)

    # Compute pairwise correlations between runs
    pairwise_correlations = []

    for run_i, run_j in combinations(range(n_runs), 2):
        rdm_i = run_rdms[run_i]
        rdm_j = run_rdms[run_j]

        # Vectorize upper triangle (exclude diagonal)
        triu_indices = np.triu_indices(n_colors, k=1)
        vec_i = rdm_i[triu_indices]
        vec_j = rdm_j[triu_indices]

        # Spearman correlation
        r, p = spearmanr(vec_i, vec_j)
        pairwise_correlations.append(r)

    result = {
        'pairwise_correlations': pairwise_correlations,
        'mean_correlation': float(np.mean(pairwise_correlations)),
        'std_correlation': float(np.std(pairwise_correlations)),
        'median_correlation': float(np.median(pairwise_correlations)),
        'min_correlation': float(np.min(pairwise_correlations)),
        'max_correlation': float(np.max(pairwise_correlations)),
        'run_rdms': run_rdms
    }

    if disparities is not None:
        result['disparities'] = disparities.tolist()
        result['mean_disparity'] = float(np.mean(disparities))

    return result


def main():
    parser = argparse.ArgumentParser(description='Compute RDM reliability with Procrustes alignment')
    parser.add_argument('--baseline-timestamp', type=str, required=True,
                       help='Timestamp of baseline results')
    parser.add_argument('--subjects', nargs='+',
                       default=[f'sub-{i:02d}' for i in range(1, 11)],
                       help='List of subjects')
    parser.add_argument('--rois', nargs='+',
                       default=['V1', 'V2', 'V3', 'hV4'],
                       help='List of ROIs')

    args = parser.parse_args()

    # Paths
    baseline_dir = Path('/scratch/connectome/haba6030/colorBlind/analysis/phase1_preprocess_decoding/method3_header_mi/results/baseline_decoding') / args.baseline_timestamp
    output_dir = Path('/scratch/connectome/haba6030/colorBlind/analysis/validation/results/rdm_reliability_aligned')

    # Create timestamp for this run
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_dir = output_dir / (args.baseline_timestamp + '_' + timestamp)
    run_dir.mkdir(parents=True, exist_ok=True)

    print(f"Output directory: {run_dir}")

    # Storage for results
    reliability_data = {
        'baseline_timestamp': args.baseline_timestamp,
        'subjects': args.subjects,
        'rois': args.rois,
        'results_before': {},
        'results_after': {}
    }

    # Analyze each subject-roi
    print("\nComputing RDM reliability (Before & After Alignment)...")
    for subject in tqdm(args.subjects, desc='Subjects'):
        reliability_data['results_before'][subject] = {}
        reliability_data['results_after'][subject] = {}

        for roi in args.rois:
            # Load amplitudes
            amp_path = baseline_dir / subject / roi / 'amplitudes_z.npy'

            if not amp_path.exists():
                print(f"Warning: No data for {subject} {roi}")
                continue

            amplitudes_z = np.load(amp_path)  # (n_runs, n_colors, n_voxels)

            try:
                # Compute reliability BEFORE alignment
                reliability_before = compute_rdm_reliability(amplitudes_z, use_alignment=False)

                # Compute reliability AFTER alignment
                reliability_after = compute_rdm_reliability(amplitudes_z, use_alignment=True)

                # Store (exclude run_rdms from JSON)
                reliability_data['results_before'][subject][roi] = {
                    'pairwise_correlations': reliability_before['pairwise_correlations'],
                    'mean_correlation': reliability_before['mean_correlation'],
                    'std_correlation': reliability_before['std_correlation'],
                    'median_correlation': reliability_before['median_correlation']
                }

                reliability_data['results_after'][subject][roi] = {
                    'pairwise_correlations': reliability_after['pairwise_correlations'],
                    'mean_correlation': reliability_after['mean_correlation'],
                    'std_correlation': reliability_after['std_correlation'],
                    'median_correlation': reliability_after['median_correlation'],
                    'disparities': reliability_after['disparities'],
                    'mean_disparity': reliability_after['mean_disparity']
                }

            except ValueError as e:
                print(f"Error: {subject} {roi} - {e}")
                print(f"  Skipping {subject} {roi} (likely data quality issue)")
                continue

    # Save detailed results
    json_path = run_dir / 'within_subject_reliability_aligned.json'
    with open(json_path, 'w') as f:
        json.dump(reliability_data, f, indent=2)
    print(f"\nSaved detailed results to {json_path}")

    # ========================================================================
    # Visualization
    # ========================================================================

    print("\nGenerating visualizations...")

    # Prepare data for plotting
    plot_data = []
    for subject in args.subjects:
        for roi in args.rois:
            if subject not in reliability_data['results_before']:
                continue
            if roi not in reliability_data['results_before'][subject]:
                continue

            before = reliability_data['results_before'][subject][roi]['mean_correlation']
            after = reliability_data['results_after'][subject][roi]['mean_correlation']
            disparity = reliability_data['results_after'][subject][roi]['mean_disparity']

            plot_data.append({
                'subject': subject,
                'roi': roi,
                'before': before,
                'after': after,
                'improvement': after - before,
                'disparity': disparity
            })

    import pandas as pd
    df = pd.DataFrame(plot_data)

    if len(df) == 0:
        print("No data available for visualization")
        return

    # Figure 1: Before vs After Comparison
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))

    # 1A: Paired comparison
    ax = axes[0]
    roi_positions = {roi: i for i, roi in enumerate(args.rois)}

    for roi in args.rois:
        roi_data = df[df['roi'] == roi]
        if len(roi_data) == 0:
            continue

        pos = roi_positions[roi]
        before_vals = roi_data['before'].values
        after_vals = roi_data['after'].values

        # Boxplots
        bp1 = ax.boxplot([before_vals], positions=[pos - 0.15], widths=0.25,
                         patch_artist=True, showfliers=False)
        bp2 = ax.boxplot([after_vals], positions=[pos + 0.15], widths=0.25,
                         patch_artist=True, showfliers=False)

        for patch in bp1['boxes']:
            patch.set_facecolor('lightcoral')
            patch.set_alpha(0.6)
        for patch in bp2['boxes']:
            patch.set_facecolor('lightgreen')
            patch.set_alpha(0.6)

        # Connect pairs with lines
        for i in range(len(before_vals)):
            ax.plot([pos - 0.15, pos + 0.15], [before_vals[i], after_vals[i]],
                   'gray', alpha=0.3, linewidth=1)

    ax.axhline(y=0.6, color='green', linestyle='--', label='Good (r>0.6)', linewidth=2)
    ax.axhline(y=0.4, color='orange', linestyle='--', label='Acceptable (r>0.4)', linewidth=2)
    ax.set_xticks(list(roi_positions.values()))
    ax.set_xticklabels(list(roi_positions.keys()))
    ax.set_xlabel('ROI', fontsize=12)
    ax.set_ylabel('Mean RDM Correlation', fontsize=12)
    ax.set_title('RDM Reliability: Before vs After Alignment', fontsize=14)
    ax.set_ylim([0, 1])
    ax.legend(['Good', 'Acceptable', 'Before', 'After'], fontsize=10)
    ax.grid(True, alpha=0.3)

    # 1B: Improvement distribution
    ax = axes[1]

    for roi in args.rois:
        roi_data = df[df['roi'] == roi]['improvement'].values
        if len(roi_data) > 0:
            pos = roi_positions[roi]
            bp = ax.boxplot([roi_data], positions=[pos], widths=0.5,
                           patch_artist=True)
            for patch in bp['boxes']:
                patch.set_facecolor('skyblue')
                patch.set_alpha(0.6)

    ax.axhline(y=0, color='gray', linestyle='-', linewidth=1)
    ax.set_xticks(list(roi_positions.values()))
    ax.set_xticklabels(list(roi_positions.keys()))
    ax.set_xlabel('ROI', fontsize=12)
    ax.set_ylabel('Improvement (After - Before)', fontsize=12)
    ax.set_title('Alignment Effect on Reliability', fontsize=14)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    fig_path1 = run_dir / 'rdm_reliability_comparison.png'
    plt.savefig(fig_path1)
    print(f"Saved comparison plot to {fig_path1}")
    plt.close()

    # Figure 2: Disparity distribution
    fig, ax = plt.subplots(figsize=(10, 6))

    for roi in args.rois:
        roi_data = df[df['roi'] == roi]['disparity'].values
        if len(roi_data) > 0:
            pos = roi_positions[roi]
            bp = ax.boxplot([roi_data], positions=[pos], widths=0.5,
                           patch_artist=True)
            for patch in bp['boxes']:
                patch.set_facecolor('plum')
                patch.set_alpha(0.6)

    ax.set_xticks(list(roi_positions.values()))
    ax.set_xticklabels(list(roi_positions.keys()))
    ax.set_xlabel('ROI', fontsize=12)
    ax.set_ylabel('Mean Disparity (lower = better alignment)', fontsize=12)
    ax.set_title('Procrustes Alignment Quality (Run-to-Run)', fontsize=14)
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    fig_path2 = run_dir / 'alignment_disparities.png'
    plt.savefig(fig_path2)
    print(f"Saved disparity plot to {fig_path2}")
    plt.close()

    # Compute summary statistics
    summary = {
        'before': {
            'mean_reliability': float(df['before'].mean()),
            'std_reliability': float(df['before'].std()),
            'median_reliability': float(df['before'].median())
        },
        'after': {
            'mean_reliability': float(df['after'].mean()),
            'std_reliability': float(df['after'].std()),
            'median_reliability': float(df['after'].median())
        },
        'improvement': {
            'mean': float(df['improvement'].mean()),
            'std': float(df['improvement'].std()),
            'median': float(df['improvement'].median())
        },
        'quality_check_before': {
            'good_count': int((df['before'] > 0.6).sum()),
            'acceptable_count': int(((df['before'] > 0.4) & (df['before'] <= 0.6)).sum()),
            'poor_count': int((df['before'] <= 0.4).sum()),
            'total_count': len(df)
        },
        'quality_check_after': {
            'good_count': int((df['after'] > 0.6).sum()),
            'acceptable_count': int(((df['after'] > 0.4) & (df['after'] <= 0.6)).sum()),
            'poor_count': int((df['after'] <= 0.4).sum()),
            'total_count': len(df)
        }
    }

    # Save results summary
    results_path = run_dir / 'results.json'
    with open(results_path, 'w') as f:
        json.dump(summary, f, indent=2)
    print(f"Saved summary to {results_path}")

    # Save settings
    settings = {
        'script': '03b_rdm_reliability_aligned.py',
        'baseline_timestamp': args.baseline_timestamp,
        'subjects': args.subjects,
        'rois': args.rois,
        'timestamp': timestamp
    }
    settings_path = run_dir / 'settings.json'
    with open(settings_path, 'w') as f:
        json.dump(settings, f, indent=2)
    print(f"Saved settings to {settings_path}")

    print("\n" + "="*70)
    print("SUMMARY")
    print("="*70)
    print(f"Before Alignment:")
    print(f"  Mean reliability: {summary['before']['mean_reliability']:.3f} ± {summary['before']['std_reliability']:.3f}")
    print(f"  Good (>0.6): {summary['quality_check_before']['good_count']}/{summary['quality_check_before']['total_count']}")
    print(f"\nAfter Alignment:")
    print(f"  Mean reliability: {summary['after']['mean_reliability']:.3f} ± {summary['after']['std_reliability']:.3f}")
    print(f"  Good (>0.6): {summary['quality_check_after']['good_count']}/{summary['quality_check_after']['total_count']}")
    print(f"\nImprovement:")
    print(f"  Mean: {summary['improvement']['mean']:.3f} ± {summary['improvement']['std']:.3f}")
    print(f"  Median: {summary['improvement']['median']:.3f}")
    print("="*70)


if __name__ == '__main__':
    main()
