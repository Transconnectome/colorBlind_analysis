#!/usr/bin/env python3
"""
Summarize T1w Space ROI Results
...
def collect_results(derivatives_dir, subjects, rois):
    ...
    for subject in subjects:
        for roi in rois:
            ...
            # Check for output files (T1w space)
            roi_t1w = subj_dir / f"sub-{subject}_{roi}_space-T1w.nii.gz"
            roi_mask = subj_dir / f"sub-{subject}_{roi}_space-T1w_mask.nii.gz"

            # QC images
            qc_overlay = subj_dir / f"QC_sub-{subject}_{roi}_overlay.png"
            qc_detailed = subj_dir / f"QC_sub-{subject}_{roi}_detailed.png"
            qc_histogram = subj_dir / f"QC_sub-{subject}_{roi}_histogram.png"
            sanity_check = subj_dir / f"sanity_check_sub-{subject}_{roi}.png"
            ...
            result = {
                ...
                'status': 'Success' if (roi_t1w.exists() and voxel_count != "N/A" and voxel_count > 0) else 'Failed'
            }
...
<body>
    <div class="container">
        <h1>🧠 T1w Space ROI Analysis Report</h1>

        <div class="metadata">
            <p><strong>Generated:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
            <p><strong>Purpose:</strong> MNI-independent ROI analysis in T1w space (1mm resolution)</p>
            <p><strong>Dataset:</strong> method3_header_mi (MI-based coregistration)</p>
        </div>
...
def main():
    ...
    parser.add_argument('--logs', type=str,
                        default='/scratch/connectome/haba6030/colorBlind/analysis/prep_trials/results/native_roi/logs',
