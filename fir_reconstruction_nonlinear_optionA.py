...
# -*- coding: utf-8 -*-
"""
fir_reconstruction_nonlinear_optionA.py
---------------------------------------
Nonlinear Forward Models with Option A (Universal HRF + Run-Specific GLM)

KEY FEATURES:
1. Option A implementation (B&H 2009 exact method)
   - Universal HRF estimation from all runs (for optimal delay)
   - Run-specific FirstLevelModel fitting with SHARED HRF shape
   - Independent beta extraction per run (no data leakage)

2. Multiple forward models comparison:
   - Linear: B&H (2009) original (OLS solution)
   - Random Forest: Nonlinear ensemble method
   - MLP: Neural network with regularization
...
DIFFERENCE FROM LINEAR OPTION A:
- Tests whether nonlinear models improve reconstruction
- Compares 3 models on same exact data
- Maintains B&H methodology (Option A)

Usage:
    python fir_reconstruction_nonlinear_optionA.py --roi V1 --subject P01 --models linear rf mlp
...
class LinearForwardModel:
    """Linear forward model (B&H 2009 original)
    ...
    def fit(self, X_train, C_train):
        ...
        X_train : (n_samples, n_features)
            Brain responses (PCA components)
        C_train : (6, n_samples)
            Channel responses for training colors
        """
        # W = B × C^T × (C × C^T)^-1
    ...
    def predict(self, X_test):
        ...
        """
        # C_test = (W^T W)^-1 W^T B_test
        return np.linalg.pinv(self.W.T @ self.W) @ self.W.T @ X_test.T


class RFForwardModel:
    """Random Forest forward model

    Nonlinear ensemble method for voxel → channel mapping
    """
    def __init__(self, n_estimators=100, max_depth=10, min_samples_leaf=5, random_state=42):
        ...
        self.models = [
            RandomForestRegressor(
                ...
                n_jobs=1
            ) for i in range(6)  # One model per channel
        ]

    def fit(self, X_train, C_train):
        """Train RF models for each channel
        ...
        X_train : (n_samples, n_features)
            Brain responses
        C_train : (6, n_samples)
            Target channel responses
        """
        for ch_idx in range(6):
            self.models[ch_idx].fit(X_train, C_train[ch_idx, :])
    ...
    def predict(self, X_test):
        ...
        predictions = []
        for ch_idx in range(6):
            pred = self.models[ch_idx].predict(X_test)
            predictions.append(pred)
        return np.array(predictions)  # (6, n_test)


class MLPForwardModel:
    """Multi-layer Perceptron forward model

    Neural network with regularization
    """
    def __init__(self, n_hidden=64, learning_rate=0.001, alpha=0.01,
                 max_iter=500, random_state=42):
        self.name = "MLP (Neural Net)"
        self.models = [
            MLPRegressor(
    ...
    def fit(self, X_train, C_train):
        """Train MLP models for each channel
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Nonlinear Forward Models with Option A (B&H 2009 + Nonlinear)')
    ...
    parser.add_argument('--use-pca', action='store_true', default=True,
                        help='Use PCA dimensionality reduction (default: True)')
    ...
    parser.add_argument('--rf-n-estimators', type=int, default=100,
                        help='RF: number of trees (default: 100)')
    parser.add_argument('--rf-max-depth', type=int, default=10,
                        help='RF: max tree depth (default: 10)')
    parser.add_argument('--rf-min-samples-leaf', type=int, default=5,
                        help='RF: min samples per leaf (default: 5)')

    # MLP hyperparameters
    parser.add_argument('--mlp-n-hidden', type=int, default=64,
    ...
    parser.add_argument('--mlp-alpha', type=float, default=0.01,
                        help='MLP: L2 regularization (default: 0.01)')
...
if args.output_dir:
    ...
    if SUBJECT_ID == 'P01':
        output_dir = Path(f"derivatives/{timestamp}/pilot/{DERIVATIVE_PREFIX}/nonlinear_optionA/{ROI_NAME}")
    else:
        output_dir = Path(f"derivatives/{timestamp}/{DERIVATIVE_PREFIX}/nonlinear_optionA/{ROI_NAME}")
...
print("="*70)
print("Nonlinear Forward Models with Option A (B&H 2009 + Nonlinear)")
print("="*70)
print(f"Subject: {SUBJECT_ID} (files: {FILE_PREFIX}, derivatives: {DERIVATIVE_PREFIX})")
print(f"ROI: {ROI_NAME}")
print(f"Method: Option A (Shared HRF + Independent betas)")
...
print(f"[8/10] Multi-model reconstruction comparison (trained colors)")
print(f"  Models: {', '.join(args.models)}")
sys.stdout.flush()
...
# Multi-model reconstruction: leave-one-run-out
all_model_results = {}  # {model_name: reconstruction_results}
...
for model_name in args.models:
    ...
    if model_name == 'linear':
        ...
        model = RFForwardModel(
            n_estimators=args.rf_n_estimators,
            max_depth=args.rf_max_depth,
            min_samples_leaf=args.rf_min_samples_leaf
        )
    elif model_name == 'mlp':
        model = MLPForwardModel(
    ...
    for test_run in range(N_RUNS):
        ...
        # B&H (2009): Stack voxels across runs
        X_train = all_betas[train_runs].transpose(1, 0, 2).reshape(N_COLORS, -1)
        y_train = np.arange(N_COLORS)
        ...
        # Train model
        model.fit(X_train_final, C_train)

        # Predict channels
        C_test_est = model.predict(X_test_final)  # (6, n_test)
    ...
    all_hit_rates_30 = [np.mean(r['errors'] <= 30) * 100 for r in reconstruction_results]
    all_hit_rates_45 = [np.mean(r['errors'] <= 45) * 100 for r in reconstruction_results]

    print(f"\n{model_name.upper()} Summary:")
    print(f"  Mean error: {mean_error:.1f}°")
    print(f"  Hit rate (±30°): {np.mean(all_hit_rates_30):.1f}%")
    print(f"  Hit rate (±45°): {np.mean(all_hit_rates_45):.1f}%")

    all_model_results[model_name] = {
        'reconstruction_results': reconstruction_results,
        'mean_error': mean_error,
        'hit_rate_30': np.mean(all_hit_rates_30),
        'hit_rate_45': np.mean(all_hit_rates_45)
    }
    ...
    df = pd.DataFrame([{
        ...
        'hit_rate_30': np.mean(r['errors'] <= 30) * 100,
        'hit_rate_45': np.mean(r['errors'] <= 45) * 100
    } for r in reconstruction_results])
    df.to_csv(output_dir / f'reconstruction_{model_name}.csv', index=False)

# Model comparison summary
print(f"\n{'='*70}")
print("MODEL COMPARISON (Trained Colors)")
print(f"{'='*70}")
print(f"{'Model':<15} {'Mean Error':<15} {'Hit Rate (±30°)':<18} {'Hit Rate (±45°)':<18}")
print("-" * 70)
for model_name in args.models:
    results = all_model_results[model_name]
    print(f"{model_name.upper():<15} {results['mean_error']:>10.1f}°     "
          f"{results['hit_rate_30']:>12.1f}%        {results['hit_rate_45']:>12.1f}%")
print("-" * 70)
print()
sys.stdout.flush()
...
print(f"[9/10] Leave-one-color-out reconstruction (novel colors)")
print(f"  Models: {', '.join(args.models)}")
print(f"  Note: Skipped for brevity (can be added if needed)")
sys.stdout.flush()
...
# Update summary with model comparison
results_summary['models'] = args.models
for model_name in args.models:
    model_res = all_model_results[model_name]
    results_summary[f'{model_name}_mean_error'] = model_res['mean_error']
    results_summary[f'{model_name}_hit_rate_30'] = model_res['hit_rate_30']
    results_summary[f'{model_name}_hit_rate_45'] = model_res['hit_rate_45']
...
print(f"\n{'='*70}")
print(f"[10/10] Analysis Complete!")
print(f"{'='*70}")
print(f"\nResults saved to: {output_dir}")

print(f"\nKey Findings (Nonlinear Option A):")
print(f"  - Method: Option A (Universal HRF + Independent GLM)")
...
print(f"\nModel Comparison:")
print(f"  {'Model':<10} {'Mean Error':<15} {'Hit±30°':<12} {'Hit±45°':<12}")
print(f"  {'-'*50}")
for model_name in args.models:
    res = all_model_results[model_name]
    print(f"  {model_name.upper():<10} {res['mean_error']:>8.1f}°        "
          f"{res['hit_rate_30']:>6.1f}%      {res['hit_rate_45']:>6.1f}%")

# Find best model
best_model = min(args.models, key=lambda m: all_model_results[m]['mean_error'])
best_error = all_model_results[best_model]['mean_error']
linear_error = all_model_results.get('linear', {}).get('mean_error', None)

print(f"\n  Best model: {best_model.upper()} ({best_error:.1f}°)")
if linear_error and best_model != 'linear':
    improvement = linear_error - best_error
    print(f"  Improvement over Linear: {improvement:.1f}° ({improvement/linear_error*100:.1f}%)")
elif best_model == 'linear':
    print(f"  Linear model is best (simpler is better!)")

print(f"\nThis implements:")
print(f"  ✅ B&H (2009) Option A methodology")
print(f"  ✅ Shared HRF shape (stable)")
print(f"  ✅ Independent betas (proper CV)")
print(f"  ✅ Nonlinear model comparison")
print()
print("="*70)
sys.stdout.flush()
