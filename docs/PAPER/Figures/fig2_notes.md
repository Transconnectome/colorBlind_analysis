...
User feedback 반영 후 다음 세 가지 변경:

1. **`n.s.` / `*` 표기 명료화** — 단일 패널-레벨 표시 → **per-subject** 표시로 변경
   - hV4 컬럼에서 각 CVD 피험자(sub-08 ■, sub-09 ▲)의 마커 색과 매칭하여 `n.s./*` + p-value 부착
   - 통계 방향:
     - Panel A (LORO, 보존성 가설): two-sided Crawford & Howell
     - Panel B (LOCO, 결손 가설): one-sided lower Crawford & Howell
   - hV4 결과:
     | Panel | sub-08 (deutan) | sub-09 (protan) |
     |---|---|---|
     | A LORO | t=1.569, p=0.168 (n.s.) | t=0.724, p=0.496 (n.s.) |
     | B LOCO | t=-1.584, p=0.082 (n.s.) | t=-2.484, p=0.024 (*) |

2. **Panel C 시각 인코딩 일관화** — A/B와 동일한 "HC=bar, CVD=marker" 규약
   - 이전: HC/sub-08/sub-09 모두 grouped bar (세 막대)
   - 변경: HC=bar (full width 0.55), sub-08=square marker + 연결선, sub-09=triangle marker + 연결선

3. **LORO/LOCO 부제 추가** — 각 패널 letter 옆에 method 설명
   - A: "Discrimination — LORO (leave-one-run-out)"
   - B: "Interpolation — LOCO (leave-one-color-out)"
   - C: "Per-hue interpolation at hV4 — LOCO"

4. **Chance line 라벨 추가** — 우측 끝에 `chance = 1/8` (A) / `chance = 3/8` (B, C) 텍스트로 표시

Layout: figsize 180×88mm → 180×96mm, gs.top 0.90 → 0.83 (subtitle 공간 확보).

References for design choices: Brouwer & Heeger (2009) J Neurosci Fig 5C (per-condition * with paired t-test), Bannert & Bartels (2018) Cereb Cortex Fig 3 (side-by-side conditions with **/n.s. anchored to bars), via NotebookLM ColorBlind_comprehensive query (2026-05-11).
