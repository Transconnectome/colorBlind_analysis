...
The 2-component grid search under the HYBRID loss (Equation~\ref{eq:hybrid}; main-text Section~\ref{sec:methods:twocomp}) was applied to each of the 7 HC participants treated as a CVD candidate.
...
Consequently, per-subject p-values reported alongside the HYBRID argmin in Figure~\ref{fig:twocomp} and Section~\ref{sec:results:twocomp} are presented as \emph{descriptive} fits of representational geometry, not as CVD-specificity statements.
Two case-level anchors support biological interpretation despite this caveat: (i) parameter-norm comparison --- both CVD subjects' $\|\hat{\beta}\|$ (43.1\degree, 32.3\degree) fall \emph{below} the HC leave-one-out range $[49.0^\circ, 65.3^\circ]$, consistent with CVD geometry requiring less aggressive 2-component correction than the HC permutation null; (ii) directional consistency with literature anchors ($\hat{\beta}_c$ sign matching Brettel's L--M polarity; $\hat{\beta}_s > 0$ in both subjects, consistent with Emery's S-cone-axis hV4 RDM finding).
