...
Consistent with this account, clinical validation of consumer color-enhancing CVD-correction glasses reports at most modest changes in chromatic discrimination thresholds in congenital red--green deficiencies~\citep{patterson2022}, and recent behavioral work likewise shows that population-average filters change the appearance of color without substantively shifting discrimination thresholds at threshold contrast~\citep{somers2024}, while the underlying CVD phenotype varies markedly even within a single diagnostic category~\citep{bosten2019}.
...
Behavioral work converges on the conclusion that anomalous trichromats do not merely discriminate less well; they compensate.
...
\emph{(Gap 1)} Existing neural studies of color characterize population-mean geometry in healthy observers~\citep{brouwer2009, bannert2018, kuriki2015} or, in CVD, group means at the level of activation magnitude.
To our knowledge --- based on PubMed and Google Scholar searches combining ``CVD'' / ``color vision deficiency'' / ``anomalous trichromacy'' with ``fMRI'', ``forward encoding'', and ``individual differences'' --- none has inverted the high-dimensional cortical signal into an interpretable, individualized distortion field.
...
We then cross-check the winning fit against two independent signatures. The first is the $\Delta$RDM, the pairwise difference of representational dissimilarity matrices (RDMs) in the common space derived by the Shared Response Model (SRM; \citeNP{chen2015}). The second is the cross-cohort S-cone compensation angle described above.
...
Retinal and cortical fits converge on \emph{who} is distorted but diverge on \emph{how} to correct --- generating a falsifiable behavioral prediction to be tested in a same-day 2AFC discrimination experiment.
\end{enumerate}
...
Case-level specificity therefore rests on joint anchors --- HC parameter-norm bracket, directional consistency with cone-physiology, and (pending) Phase-3 behavioral validation --- rather than on per-case $p$-values.
