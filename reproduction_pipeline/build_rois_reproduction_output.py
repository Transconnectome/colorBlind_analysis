...
# -*- coding: utf-8 -*-
"""
build_rois_reproduction_output.py
----------------------------------
Build ROI masks using OLD OUTPUT directory structure.

This is identical to build_rois_reproduction.py but uses
config_reproduction_output.py instead of config_reproduction.py

Usage:
    python build_rois_reproduction_output.py
    python build_rois_reproduction_output.py --subject 02
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Build ROI masks for reproduction pipeline (OUTPUT directory)'
    )
    parser.add_argument(
        '--subject',
        type=str,
...
def find_reference_image(subject_id):
    ...
    IMPORTANT: Always use run-1 for consistency!
    Different runs give different EPI masks even in same MNI space.
    """
    ...
    # CRITICAL: Use run-1 specifically (not just any run!)
    # Different runs have different brain coverage masks
    pattern = f"sub-*_task-rsvp_run-1_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz"
    candidates = list(func_dir.glob(pattern))

    if not candidates:
        print(f"  [WARN] Run-1 not found, trying any run...")
    ...
    # Warn if not run-1
    if "run-1" not in ref_path.name:
        print(f"    ⚠ WARNING: Using {ref_path.name} instead of run-1!")
        print(f"    Different runs may give different EPI masks!")
...
def main():
    ...
    print("="*70)
    print("ROI Mask Creation for Reproduction Pipeline (OUTPUT Directory)")
    print("="*70)
    ...
    # Using run-1 consistently ensures reproducible brain mask
    print("\nComputing EPI mask from functional data (run-1)...")
    ref_img = nib.load(ref_img_path)
    ...
    print(f"  Note: Using run-1 for consistent mask across analyses")
    ...
    if roi_counts:
        ...
        print(f"Next step: Run run_reconstruction_reproduction_output.py")
