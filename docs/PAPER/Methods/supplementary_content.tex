...
% ========== Section 15: Why fit-validity permutation p-values are not reported ==========

\subsection*{\textbf{S15. Why per-subject fit-validity permutation $p$-values are not reported}}
\label{app:hc_specificity}

Label-permutation $p$-values for the per-subject $(\hat\beta_s, \hat\beta_c)$ argmin assess whether the loss landscape contains a non-trivial minimum on a single participant's vulnerability profile under random permutations of the 8-color label assignment.
They do not assess whether the inverted filter $\delta\theta^{\rm filt}_k = \tilde\theta_k - \theta_k$ restores HC-equivalent perception at the stimulus level.
Because filter validity is the substantive claim of this paper, fit-landscape permutation $p$-values are omitted from the main text and the per-subject $(\hat\beta_s, \hat\beta_c)$ are reported as descriptive fits.
Quantitative filter validity is established by the pre-registered Phase-3 2AFC discrimination arm (see the \nameref{sec:methods:behavioral} section, last paragraph).
...
\begin{table}[ht]
  ...
  Fit-validity permutation $p$-values are not reported (see Supplementary~\ref{app:hc_specificity}).}
  \label{tab:S1}
  \centering
  \small
...
Each of $n = 6$ effective HC participants (excluding sub-07 for low hV4 voxel count) was refit under the canonical V4 LOCO loss $L_\text{fit}$ (Equation~\ref{eq:lfit}) with the remaining HC pool as reference, yielding an HC leave-one-out distribution of fitted-parameter magnitudes $\|\hat\beta\|$ ranging $49.0^\circ$ to $65.3^\circ$ (mean $54.8^\circ$).
This range is reported as a magnitude anchor for the CVD estimates (sub-08 $\|\hat\beta\| = 40.5^\circ$; sub-09 $\|\hat\beta\| = 22.8^\circ$, both below the HC range), not as a hypothesis test of CVD-versus-HC specificity.
Within-cohort permutation $p$-values for the per-subject argmin are omitted from the main text for the reasons given in Supplementary~\ref{app:hc_specificity}.
