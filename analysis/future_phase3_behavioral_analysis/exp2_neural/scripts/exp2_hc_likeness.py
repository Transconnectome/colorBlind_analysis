...
def loco_rho_within(amp, C_baseline):
    """Within-subject leave-one-color-out rho at delta_theta=0.
    amp: (n_runs, 8, V). Pooled-run training (NOT run-averaged), per-color holdout.
    Returns rho (8,)."""
    n_runs, n_colors, V = amp.shape
    amp_mean = amp.mean(axis=0)  # (8, V)
    rho = np.zeros(n_colors)
    for c in range(n_colors):
        train = [k for k in range(n_colors) if k != c]
        C_train = np.tile(C_baseline[train], (n_runs, 1))     # (n_runs*7, K)
        X_train = amp[:, train, :].reshape(-1, V)             # (n_runs*7, V)
        alpha, _ = gcv_select_alpha(C_train, X_train)
        W = fit_W_ridge(C_train, X_train, alpha)              # (K, V)
        Y_pred = (C_baseline[c:c + 1] @ W)[0]                 # (V,)
        Y_act = amp_mean[c]                                   # (V,)
        if np.std(Y_pred) < 1e-10 or np.std(Y_act) < 1e-10:
            rho[c] = 0.0
        else:
    ...
    for roi in ROIS:
        ...
        if nf_amp is not None:
            ...
            roi_res['nofilter_baseline_exp1'] = {
                ...
                'loco_d_vs_hc_n4': nf_d, 'loro_acc': float(loro_accuracy(nf_amp)),
            ...
            print(f"  [nofilter-exp1] LOCO rho n4={nf_rho4:.4f} n6={nf_rho6:.4f} "
                  f"(d_vs_HCn4={nf_d:+.2f}) V={nf_amp.shape[2]}")
        ...
        if 'window' in cond_rho and 'optimal' in cond_rho:
            ...
            if np.isfinite(nf_rho4):
                roi_res['contrast_optimal_minus_window']['vs_nofilter'] = {
