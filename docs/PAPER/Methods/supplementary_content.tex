...
The regularization parameter $\alpha$ for ridge regression was selected via Generalized Cross-Validation (GCV; \citeNP{golub1979}).
...
The 2-component grid-search pipeline was applied to the 7 HC participants treating each as a CVD candidate.
Of 21 model--subject tests (7 HC $\times$ 3 models), 15 reached nominal $p < 0.05$ (71\%).
This elevated false-positive rate reflects (i) rank-based loss with only 8 data points (Spearman $\rho$ over 8 hues is sensitive to individual rank fluctuations), and (ii) regression to the mean on per-HC baseline $\rho$ (hV4 $K = 3$, 63 voxels; individual HC baselines span $[-0.36, +0.69]$).
Case-level specificity therefore rests on the three converging anchors described in Section~\ref{sec:results:filter}, not on per-subject permutation significance alone.
...
\textbf{Fits (Table~\ref{tab:S1}).} Applying a closed-testing rule (Machado first; R+C escalated only if Machado $p > 0.05$), the retinal family reaches significance for both subjects but selects a different model class per case.
Sub-08 escalates to R+C ($\Delta\lambda = 2.0$~nm, $g = +2.25$, $\rho = 0.857$, $p = 0.005$) because 1-DOF Machado is trend-level ($p = 0.058$).
Sub-09 is captured by 1-DOF Machado ($\Delta\lambda = 13.5$~nm, $\rho = 0.762$, $p = 0.018$) without cortical augmentation; the shift magnitude falls within the established range for moderate--severe protanomaly~\cite{neitz2011}.
...
Retinal-family optimisation directly against $\Delta$RDM fails for both CVD subjects; LOCO is the more discriminating phenotype and $\Delta$RDM serves as a convergence cross-check only.
...
\begin{table}[ht]
  ...
  Retinal family: closed-testing (Machado first; R+C only when Machado $p > 0.05$). $p$-values: $8! = 40{,}320$ exact permutations of colour labels on Spearman $\rho$.}
  \label{tab:S1}
  \centering
  \small
  \begin{tabular}{llccccc}
    \toprule Subject & Model & DOF & Param.\ 1 & Param.\ 2 & $\rho$ & $p_{\text{perm}}$ \\ \midrule Sub-08 (deutan) & 2-component & 2 & $\beta_s = 38^\circ$ & $\beta_c = -14^\circ$ & 0.881 & \textbf{0.004} \\ Sub-09 (protan) & 2-component & 2 & $\beta_s = 6^\circ$ & $\beta_c = -22^\circ$ & 0.690 & \textbf{0.035} \\ \midrule Sub-08 (deutan) & R+C & 2 & $\Delta\lambda = 2.0$~nm & $g = +2.25$ & 0.857 & \textbf{0.005} \\ Sub-09 (protan) & Machado & 1 & $\Delta\lambda = 13.5$~nm & --- & 0.762 & \textbf{0.018} \\ \bottomrule
  \end{tabular}
\end{table}

% ========== Section 17: HC Permutation under the V4 LOCO Loss ==========

\subsection*{\textbf{S17. HC Permutation under the V4 LOCO Loss}}
\label{app:hc_perm}

The 2-component grid search under the V4 LOCO loss $L_\text{fit}$ (Equation~\ref{eq:lfit}; main-text Section~\ref{sec:methods:twocomp}) was applied to each of the 7 HC participants treated as a CVD candidate.
Of the 6 HCs that pass the hV4 voxel-count threshold (sub-07 excluded; 16 voxels, $K = 3$ produces \texttt{nan}), all 6 reach nominal $p < 0.05$ under the label-permutation null --- and the property generalises to the full HC pool (HC FPR = 7/7 at nominal $\alpha = 0.05$).

This high false-positive rate cannot be removed by reformulating the fitting criterion: thirteen successive reformulations within the voxel-prediction loss family ($L_\mathrm{LOCO}$, $L_\mathrm{mse}$, $L_\mathrm{rdm}$, cross-ROI combinations, baseline-$\rho$ correction, top-$K$ Jaccard) all yielded the same outcome (project log, Cycles 9--13).
It reflects two structural properties of the available data: (i) the 8-hue resolution provides limited degrees of freedom for distinguishing specific from non-specific fits at the single-subject level, and (ii) the relevant null hypothesis (``no CVD-specific distortion'') is not isomorphic to label permutation on a single participant's vulnerability profile, since a non-trivial fit can be obtained for any sufficiently smooth 8-point pattern under the 2-DOF model class.

Consequently, per-subject p-values reported alongside the $L_\text{fit}$ argmin in Figure~\ref{fig:twocomp} and Section~\ref{sec:results:twocomp} are presented as \emph{descriptive} fits of representational geometry, not as CVD-specificity statements.
Two case-level anchors support biological interpretation despite this caveat: (i) parameter-norm comparison --- both CVD subjects' $\|\hat{\beta}\|$ ($40.5^\circ$ for sub-08; $22.8^\circ$ for sub-09) fall \emph{below} the HC leave-one-out range $[49.0^\circ, 65.3^\circ]$, consistent with CVD geometry requiring less aggressive 2-component correction than HC label-permutation nulls; (ii) per-subject R+C diagnostic decomposition (Appendix~\ref{app:altmodels}) recovers cortical-dominant etiology for sub-08 and retinal-dominant etiology for sub-09, providing a mechanistic interpretation of the 2-component fits that is not tested by the within-cohort permutation null.
Quantitative specificity claims require an external behavioural endpoint such as the Phase-3 2AFC arm (Methods~\ref{sec:methods:behavioral}, last paragraph; data not yet collected at the time of writing).
