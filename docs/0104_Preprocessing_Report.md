...
## 7. 파인만 기법: 일상 비유로 이해하는 4가지 Registration 방법

**날짜**: 2026-01-06
**목적**: 복잡한 기술 개념을 직관적으로 이해하기
...
```
두 개의 투명한 지도를 정확히 겹쳐야 합니다.

지도 1 (BOLD 뇌 영상):
  - 29.5° 기울어져 있음 (oblique)
  - 뒷머리 부분만 있음 (후두엽, 전체의 1/4)
  - 희미하고 흐릿함
...
### 7.2. (a)단계와 (b)단계: 두 가지 작업

#### **(a) 초기 정렬 = "대충 맞추기"**

```
목표: 두 지도가 대략 같은 방향을 보도록
...
### 7.3. 방법 1: FLIRT → BBR (original_v3) = "패턴 매칭 → 경계선 맞추기"

#### **(a) FLIRT = 퍼즐 조각 맞추듯이 패턴 매칭**
...
1. 두 지도를 겹쳐놓고
2. 여러 각도로 돌려보면서 (0°, 15°, 30°, 45°...)
3. "아, 이 각도에서 비슷한 무늬가 가장 많이 겹치네!"
4. 그 각도로 고정

원리:
  → 밝기 패턴(intensity)이 비슷할수록 점수 높음
  → 모든 각도 시도 (360° 회전하며 테스트)
  → 가장 점수 높은 각도 선택
  → Mutual Information: "두 지도가 얼마나 비슷한 정보 공유?"
```
...
BOLD와 T1w 둘 다 이 패턴 가짐:
  → 29.5° 돌렸을 때 패턴이 가장 많이 겹침
  → FLIRT가 자동으로 찾아냄!
```

**우리 문제는?**
```
문제: 후두엽 지도만 있음 (전체의 1/4)
  → 패턴 정보가 적음
  → 잘못된 각도 선택할 위험 ("Limited FOV 문제")

하지만 우리는 운 좋게 성공!
  → Dice 0.889
  → 후두엽 패턴만으로도 충분했음
  → FLIRT의 wide search가 29.5° obliquity 찾아냄
```

#### **(b) BBR = 건축 도면처럼 경계선 맞추기**

**일상 비유:**
```
건축 도면 겹치는 것처럼:

1. FLIRT가 대충 맞춘 상태에서 시작
2. 확대경으로 건물 벽 경계선 보기
3. 벽(백질/회백질 경계)이 정확히 겹치도록 미세 조정

원리:
  → 백질(WM)과 회백질(GM) 경계선 찾기
  → BOLD 신호가 경계에서 급변함
  → 경계선이 정확히 겹치도록 조정
  → Sub-mm (0.1mm) 정밀도
```
...
문제: 후두엽은 경계선(백질) 적음
  → 소뇌가 많은데, 소뇌는 대뇌 아님
  → BBR이 헷갈릴 수 있음 ("Limited cortex 문제")

하지만:
  → FLIRT가 이미 대충 잘 맞춰놨음 (±5-10°)
  → BBR은 미세 조정만 하면 됨 (±5° 안에서만)
  → 성공!
```

**결과:**
```
✅ Dice 0.889 (Excellent!)
✅ 8명 group-level 가능

파인만 요약:
  "360° 다 돌려보며 패턴 찾기 → 확대경으로 경계선 맞추기"

  장점: Robust, 이미 성공
  단점: 느림 (FLIRT 1-2분 + BBR 1-2분), Sub-06/07 일부 낮음
```

---

### 7.4. 방법 2: Header → BBR = "자 대고 각도재기 → 경계선 맞추기"
...
1. 지도 1 모서리 각도: 29.5° (BOLD qform에 기록됨)
2. 지도 2 모서리 각도: 0° (T1w cardinal)
3. "그럼 29.5° 반대로 돌리면 되겠네!"
4. 돌림 (패턴 매칭 안 함, 빠름!)

원리:
  → NIfTI header에 회전 각도 저장되어 있음
  → 그걸 읽어서 반대로 회전
  → FLIRT처럼 모든 각도 안 돌려봄
```
...
각도기가 부정확할 수 있음:
  - 실제 obliquity: 29.5°
  - Header 기록: 27°? 32°?
  - 오차: ±2-5° (Scanner calibration에 따라)

→ 대충 맞추기(a단계)도 정확하지 않음!
→ ±5° 오차 가능
```
...
**일상 비유:**
```
잘못 맞춰진 상태에서 경계선 맞추려 함:

1. Header로 27° 돌렸는데 실제 29.5°였음 (2.5° 오차)
2. BBR: "경계선 찾아서 2.5° 더 돌려야지"
3. 하지만 BBR은 ±5° 안에서만 찾음
4. 운 좋으면 성공 (2.5° < 5°)
5. 오차 크면 실패 (6° > 5°)
```
...
예상:
  Best case: Dice 0.88-0.92 (header 정확 시)
  Likely: Dice 0.80-0.85 (일부 error)
  Worst: Dice < 0.80 (완전 실패)
```
...
장점: 빠름 (FLIRT 스킵)
단점: 자(header) 부정확하면 위험

권장하지 않는 이유:
  1. 29.5° 큰 각도 → header error 클 것
  2. original_v3 이미 좋음 (Dice 0.889)
  3. 위험 > 보상
```

---

### 7.5. 방법 3: mri_coreg (Header → MI) = "자로 대충 → 패턴 정밀 매칭 (경계선 안 씀!)"

#### **(a) Header = 자로 대충 시작**

(방법 2와 동일)

#### **(a') MI Optimization = 패턴으로 정밀하게 맞추기**

**일상 비유:**
```
자로 대충 맞춘 후, 패턴으로 정밀하게:

1. Header로 27° 돌림 (실제 29.5°, 2.5° 오차)
2. "아직 패턴이 안 맞네"
3. 근처 각도들 시도: 26°, 27°, 28°, 29°, 30°... (좁은 범위만!)
4. 29.5°에서 "패턴 완벽히 맞음!"
5. 정밀 정렬 완료!

원리:
  → Header 근처에서만 찾음 (빠름, FLIRT보다 빠른 시작)
  → 패턴 매칭 사용 (정확함, Mutual Information)
  → 경계선 안 씀 (후두엽에 좋음!)
  → Powell's method: 언덕 오르듯이 점수 높은 방향 찾기
```
...
패턴 매칭(MI) 정확도:
  - FLIRT (대충): ±5-10° 오차
  - MI optimization (정밀): ±0.5-1mm 오차

fMRI 해상도: 2mm × 2mm × 2mm
  → 1mm 오차면 충분히 정밀함!
  → 경계선(BBR 0.1mm) 필요 없음
  → 경계선 적은 후두엽에 완벽!
```
...
장점:
  ✅ 후두엽만 있어도 OK (패턴 매칭)
  ✅ 29.5° 각도 자동으로 찾음 (MI robust)
  ✅ 경계선 적어도 OK (안 씀)
  ✅ 소뇌 많아도 OK (패턴만 사용)

예상:
  → Dice 0.90-0.95 (개선!)
  → Sub-06/07 모든 runs 사용 가능

Trade-off:
  정밀도: 1mm (vs BBR 0.1mm)
  하지만 fMRI 2mm → 1mm 충분!
```

**파인만 요약:**
```
"자로 대충 → 근처에서 패턴으로 정밀하게 (경계선 안 씀!)"
...
### 7.6. 방법 4: bbregister --no-pass1 = "자로 맞추기 → 확대경만 (2단계 중 1단계만)"

#### **(a) Header = 자로 맞추기**

(방법 2, 3과 동일)

#### **(b) BBR Pass 2만 = 확대경으로 바로 정밀 조정**

**일상 비유:**
```
보통 경계선 맞추기 2단계:
  Pass 1: 손으로 대충 경계선 맞추기 (±15° 찾음, 흐릿하게 봄)
  Pass 2: 확대경으로 정밀 조정 (±5° 찾음, 선명하게 봄)
...
Pass 1 (대충 보기):
  → 해상도 낮춰서 봄 (더 흐릿함, 2배 down-sample)
  → 경계선 더 안 보임
  → 소뇌를 대뇌로 착각!
  → 잘못된 위치로 이동
...
BUT:
  → Header 정확해야만 가능! (±5° 이내)
  → 우리: 29.5° obliquity → header error 클 것
```
...
결론:
  → 먼저 각도 펴야 함 (deoblique preprocessing)
  → 또는 방법 3 (MI) 사용 (더 robust)
```

**파인만 요약:**
```
"자로 맞추기 → 확대경으로 바로 정밀 조정"

장점: 소뇌 착각 방지
단점: 자(header) 정확해야만 가능
...
```
문제:
  기울어진 작은 지도를 큰 지도에 맞추기
  - 작은 지도: 29.5° 기울어짐, 뒷머리만
  - 큰 지도: 정상 방향, 전체 뇌

방법 1 (현재):
  "돌림판 360° 다 돌려가며 맞는 각도 찾기 → 확대경"
  → 운 좋게 성공! (Dice 0.889)
  → 하지만 가끔 실패 (Sub-06/07)
...
| 방법 | (a) 초기 정렬 | (b) 미세 조정 | 비유 |
|------|-------------|-------------|------|
| **1: FLIRT→BBR** | 돌림판 360° 패턴 매칭 | 확대경으로 경계선 | 퍼즐 맞추기 → 건축 도면 |
| **2: Header→BBR** | 자로 각도 재기 | 확대경으로 경계선 | 각도기 → 건축 도면 |
| **3: Header→MI** ⭐ | 자로 대충 | 근처 패턴 정밀 | 각도기 → 퍼즐 정밀 |
| **4: Header→BBR1** | 자로 각도 재기 | 확대경만 (손 스킵) | 각도기 → 확대경만 |
...
## 8. qform 분석 및 예측 업데이트

**날짜**: 2026-01-06
**목적**: 실제 데이터의 header 정보 분석 및 예측 수정

---

### 8.1. 실제 qform 정보 (Original data without preprocessing)

#### **BOLD qform (Oblique)**

```
qto_xyz matrix:
  Row 1 (i-axis): -1.999061   0.047621  -0.038556   96.264824
  Row 2 (j-axis):  0.013467  -0.886262  -1.792863   34.247570
  Row 3 (k-axis):  0.059775   1.792281  -0.885525 -117.290436
  Row 4:           0.000000   0.000000   0.000000    1.000000

해석:
  - Voxel size: ~2mm isotropic
  - Rotation matrix (normalized):
      i-axis: [-0.9995,  0.0238, -0.0193]
      j-axis: [ 0.0067, -0.4431, -0.8964]
      k-axis: [ 0.0299,  0.8961, -0.4428]

  - j, k axes가 주로 y-z plane에서 회전됨
  - Sagittal tilt (calcarine sulcus에 직교하도록)
  - 큰 oblique rotation 확인!
```
...
```
qto_xyz matrix:
  Row 1 (i-axis):  0.999413   0.000561   0.017123 -113.533020
  Row 2 (j-axis): -0.002245   0.499731   0.016374 -117.571609
  Row 3 (k-axis): -0.034192  -0.016402   0.499438 -137.645569
  Row 4:           0.000000   0.000000   0.000000    1.000000

해석:
  - Voxel size: 1mm × 0.5mm × 0.5mm (high-res T1w)
  - Rotation matrix (normalized):
      i-axis: [ 0.9994, -0.0022, -0.0342]
      j-axis: [ 0.0011,  0.9995, -0.0328]
      k-axis: [ 0.0343,  0.0328,  0.9988]

  - 거의 identity matrix!
  - Cardinal axes (표준 방향, ~0° rotation)
  - Off-diagonal 요소 매우 작음 (< 0.035)
```

---

### 8.2. Header-based Transform 예측

#### **Computed Transform (BOLD → T1w)**

```python
# Header transform = T1w_qform^-1 @ BOLD_qform

T_bold_to_t1w ≈ [
  [ 0.9995, -0.0067, -0.0299,  tx],
  [-0.0238, -0.4431, -0.8961,  ty],
  [ 0.0193, -0.8964, -0.4428,  tz],
  [ 0,       0,       0,        1 ]
]

해석:
  - Large rotation in j-k plane
  - Obliquity ~29.5° (sagittal tilt)
  - Translation from scanner coordinates
```
...
```
✅ 좋은 소식:
  1. T1w는 거의 cardinal (표준 방향)
     → Transform 계산 간단

  2. BOLD qform에 rotation 정보 명확히 있음
     → j, k axes rotation 큼
     → 29.5° obliquity 표현됨

  3. Header에 필요한 정보 "존재"
     → --bold2anat-init header 작동 "가능"

⚠️ 불확실성:
  1. qform 정확도: Scanner calibration에 의존
     → ±2-5° error 가능

  2. sform vs qform 차이 가능
     → NIfTI 파일마다 다를 수 있음

  3. EPI distortion은 qform에 반영 안 됨
     → 기하학적 왜곡은 별도 처리 필요

→ Header-based initialization 작동 가능성 있음!
→ 하지만 정확도는 실제 테스트 필수!
```

---

### 8.3. 예측 업데이트 (qform 분석 후)

#### **방법 2 (--bold2anat-init header) 예측 변경**
...
이유:
  - "Obliquity 크면 header 부정확할 것"
  - "29.5° 큰 각도 → header 신뢰 어려움"
...
**수정된 예측 (qform 본 후):**
```
옵션 2 (--header): ⚠️ 테스트 가치 있음!

이유:
  ✅ qform에 rotation 정보 명확
     - j-axis: (0.013, -0.886, -1.793)
     - k-axis: (0.060, 1.792, -0.886)
     - 29.5° sagittal tilt 표현됨

  ✅ T1w qform 거의 cardinal
     - Off-diagonal < 0.035
     - Transform 계산 간단

  ⚠️ 하지만 여전히 불확실:
     - qform 정확도: Scanner에 따라 ±2-5°?
     - EPI distortion 미반영
     - sform vs qform 차이 가능

예상 결과 (범위 넓어짐):
  Best case: Dice 0.88-0.92 (header 정확 시)
  Likely: Dice 0.80-0.85 (일부 error)
  Worst: Dice < 0.80 (EPI distortion 큼)

작동 가능성: 30% → 50%로 상승
```

#### **방법 3 (mri_coreg) 예측 유지**

```
옵션 3 (mri_coreg): ✅ 여전히 1순위

이유:
  ✅ Header 정확해도 MI가 더 robust
     - Header init (빠른 시작)
     - MI optimization (정밀 조정)
     - EPI distortion 보정 가능

  ✅ Limited cortex 문제 해결
     - BBR 안 씀 → WM boundary 불필요
...
#### **방법 4 (bbregister --no-pass1) 예측 유지**

```
옵션 4 (--no-pass1): ⚠️ 조건부 (변동 없음)

이유:
  ⚠️ Header 정확성에 극도로 의존
     - BBR pass 2: ±5° search range only
     - qform error > 5° → 완전 실패
...
### 8.4. qform 분석이 바꾼 것 vs 바꾸지 못한 것

#### **변경된 것: 방법 2 평가**

```
Before qform analysis:
  방법 2: ❌ 명확히 비추천
  이유: "Header 부정확할 것"

After qform analysis:
  방법 2: ⚠️ 테스트 가치 있음
  이유: "Header 정보 존재, 하지만 정확도 불확실"

변경 이유:
  → qform에서 29.5° rotation 정보 확인됨
  → Header-based initialization 작동 "가능성" 확인
  → 하지만 정확도는 여전히 알 수 없음
  → 실제 테스트로 확인 필요!
```
...
이유:
  1. Header 정확해도 MI가 더 robust
     - Header: qform 의존 (±2-5° error)
     - MI: 패턴 매칭 (±0.5-1mm accuracy)

  2. Limited FOV/cortex 문제
     - Header → BBR: WM boundary 필요 → 실패 위험
     - Header → MI: 패턴만 → robust

  3. EPI distortion 보정
     - Header: 반영 안 됨
     - MI: 보정 가능 (intensity-based)

결론:
  qform 분석으로 방법 2 가능성 상승했지만,
  방법 3이 여전히 최선!
```
...
```
Before qform:
  "방법 2는 아마 실패할 것, 방법 3만 테스트해도 됨"

After qform:
  "방법 2도 성공할 수 있음, 둘 다 테스트 필요!"

실험 설계:
  ✅ 방법 2 (Header→BBR): 반드시 테스트
  ✅ 방법 3 (Header→MI): 반드시 테스트
  ⚠️ 방법 4 (BBR 1-pass): 조건부 (방법 2 성공 시)
...
**Scenario 2: 방법 2 성공 (가능성 30% ← qform으로 상승!)**
```
결과:
  - 방법 2: Dice 0.88-0.92
  - 방법 3: Dice 0.90-0.95
...
### 8.6. 최종 권장사항 (qform 분석 후)

#### **실험 우선순위**

```
1순위: 방법 3 (Header→MI) ⭐
  이유: 가장 robust, 최고 성능 예상
  예상: Dice > 0.90

2순위: 방법 2 (Header→BBR) ⭐⭐ (중요도 상승!)
  이유: qform 정보 존재, 작동 가능성 확인
  예상: Dice 0.80-0.92

3순위: 방법 1 (FLIRT→BBR) - Baseline
  이유: 이미 결과 있음 (Dice 0.889)
  목적: 비교 기준

4순위: 방법 4 (BBR 1-pass) - 조건부
  이유: 방법 2 성공 시에만 가치
  조건: IF 방법 2 Dice > 0.85
```

#### **실험 실행 전략**

```
Phase 1: 방법 2, 3 병렬 실행 (필수)
  → Sub-01, 03, 06
  → 예상 시간: 3-4 hours (병렬)
  → 목적: Header 정확도 실증 확인
...
Phase 3: 조건부 Phase (optional)
  IF 방법 2 Dice > 0.85:
    → 방법 4 실행 (bbregister --no-pass1)
  ELSE:
    → 방법 4 스킵

Phase 4: 최종 선택
  → 최고 성능 방법 채택
  → 전체 10명 재실행 또는 Sub-06/07만
```
...
```
✅ Header에 rotation 정보 존재
  - BOLD qform: 명확한 oblique rotation
  - T1w qform: 거의 cardinal
  - Transform 계산 가능

✅ Header-based initialization 작동 "가능"
  - 방법 2, 4 시도 가치 있음
  - 완전히 배제할 수 없음
```
...
```
❓ Header 정확도
  - qform error: ±2-5°? (Scanner 의존)
  - sform vs qform 차이?
  - EPI distortion 미반영

❓ BBR search range 충분성
  - Header error > 5° 시 BBR 실패
  - 우리 케이스: 아슬아슬
...
| 방법 | 이전 예상 | qform 후 예상 | 변화 |
|------|----------|-------------|------|
| 방법 1 (FLIRT→BBR) | 0.889 | 0.889 | 동일 (이미 확인) |
| 방법 2 (Header→BBR) | < 0.85 | 0.80-0.92 | ⬆️ 상승 (가능성 인정) |
| 방법 3 (Header→MI) | > 0.90 | > 0.90 | 동일 (여전히 최선) |
| 방법 4 (BBR 1-pass) | < 0.80 | < 0.80 | 동일 (여전히 위험) |

**최종 메시지:**
```
qform 분석으로:
  → 방법 2 가능성 발견 (긍정적!)
  → 하지만 방법 3 여전히 최선 (변화 없음)
  → 둘 다 테스트로 실증 필요 (중요!)
```
...
**작성 일자**: 2026-01-06
**업데이트**: 파인만 기법 설명 및 qform 분석 추가
**상태**: ✅ 직관적 이해 및 실증 예측 완료
