...
# -*- coding: utf-8 -*-
"""
fir_reconstruction_single_delay.py
-----------------------------------
Single-delay FIR reconstruction (REDUCED PARAMETERS)

Uses only peak delay (3 TRs = 4.5s) instead of full FIR time course.
This dramatically reduces parameters to help generalization.

Combines:
- Single delay FIR (reduced parameters vs full time course)
...
# FIR parameters - SINGLE DELAY ONLY
FIR_DELAYS = [3]  # Only peak delay at 4.5s (dramatically reduces parameters!)
...
output_dir = Path(f"derivatives/sub-{cfg.SUB_ID}/fir_reconstruction/{ROI_NAME}_single_delay")
...
print("="*70)
print("FIR-based Color Reconstruction Pipeline (SINGLE DELAY)")
print("="*70)
print(f"Subject: sub-{cfg.SUB_ID}")
print(f"ROI: {ROI_NAME}")
print(f"FIR delays: {FIR_DELAYS} (REDUCED PARAMETERS)")
...
time_points = np.array(FIR_DELAYS) * cfg.TR

for color_idx in range(cfg.N_COLORS):
    ax.plot(time_points, mean_responses[color_idx],
            label=f'color_{color_idx+1}', alpha=0.7)

ax.axhline(y=0, color='k', linestyle='--', alpha=0.3)
ax.axvline(x=PEAK_DELAY * cfg.TR, color='r', linestyle='--', alpha=0.3,
           label=f'Peak delay ({PEAK_DELAY * cfg.TR}s)')
ax.set_xlabel('Time (seconds)')
ax.set_ylabel('Mean response amplitude (% signal change)')
ax.set_title(f'Mean HRF from FIR estimation - {ROI_NAME}')
ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
ax.grid(True, alpha=0.3)

hrf_fig_path = fig_dir / f"{ROI_NAME}_mean_hrf.png"
plt.tight_layout()
...
print("="*70)
print("FIR Reconstruction Complete (SINGLE DELAY)")
print("="*70)
print()
print(f"ROI: {ROI_NAME}")
