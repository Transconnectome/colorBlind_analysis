...
def build_fir_design_matrix(onsets, n_scans, tr, fir_delays):
    ...
    Returns:
    --------
    X : ndarray, shape (n_scans, len(fir_delays))
        FIR design matrix
    """
...
for run in range(1, N_RUNS + 1):
    ...
    # Load confounds (motion parameters)
    ...
    # Extract 6 motion parameters (standard minimal set)
    motion_cols = ['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z']
    confounds_subset = confounds[motion_cols]

    # Drop initial volumes from confounds to match functional data
    if VOLS_TO_DROP > 0:
    ...
    # Clean the signal: confounds regression + detrending + high-pass filtering
    func_data_cleaned = clean(
        func_data,
        confounds=confounds_subset.values,
        detrend=True,              # Remove linear/polynomial drift
        standardize=False,         # Keep original scale (will z-score later)
        high_pass=1/128.0,         # High-pass filter at 128s (standard for fMRI)
        t_r=TR
    )

    all_func_data.append(func_data_cleaned)
    ...
    print(f"    Confounds: {confounds_subset.shape[0]} rows, {confounds_subset.shape[1]} regressors (6 motion params)")
    print(f"    Cleaning: detrend=True, high_pass=1/128Hz, confounds regression applied")
    print(f"    Signal change: raw std={np.std(func_data):.2f} → cleaned std={np.std(func_data_cleaned):.2f}")
...
for voxel_idx in range(n_voxels_total):
    ...
    # Estimate HRF using pseudo-inverse: h_v = pinv(X) @ y
    try:
