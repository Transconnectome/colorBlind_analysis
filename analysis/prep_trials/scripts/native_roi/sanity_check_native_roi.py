#!/usr/bin/env python3
"""
Functional sanity check for ROI in T1w space.
...
def create_diagnostic_plots(stats_dict, trial_responses, output_dir, subject, roi, run):
    ...
    fig.suptitle(f'sub-{subject} {roi} Functional Diagnostics (T1w Space)',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()

    output_path = output_dir / f"sanity_check_sub-{subject}_{roi}.png"
...
def main():
    ...
    bold_file = fmriprep_func_dir / f"sub-{args.subject}_task-rsvp_run-{args.run}_space-T1w_desc-preproc_bold.nii.gz"
    roi_mask = deriv_dir / f"sub-{args.subject}_{args.roi}_space-T1w_mask.nii.gz"
    events_file = bids_func_dir / f"sub-{args.subject}_task-rsvp_run-{args.run}_events.tsv"

    # Check BOLD file exists
    if not bold_file.exists():
        print(f"❌ T1w space BOLD file not found: {bold_file}")
        return 1
    ...
    print("=" * 80)
    print("Functional Sanity Check - T1w Space ROI")
    print("=" * 80)
    ...
    print(f"Analysis space: T1w (1mm resolution)")
    print()
    ...
    if (stats_dict['mean_tsnr'] > 20 and
        ...
        print("   → T1w space analysis can proceed")
