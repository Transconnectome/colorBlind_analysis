#!/usr/bin/env python3
"""
Visualize ROI overlay on native BOLD space for quality control.

This script creates detailed visualization of ROI masks overlaid on native
BOLD reference images to verify proper alignment in occipital cortex.
...
def main():
    ...
    # Input files
    roi_prob = subj_deriv_dir / f"sub-{args.subject}_{args.roi}_space-bold_run-{args.run}.nii.gz"
    roi_mask = subj_deriv_dir / f"sub-{args.subject}_{args.roi}_space-bold_run-{args.run}_mask.nii.gz"
    bold_ref = fmriprep_func_dir / f"sub-{args.subject}_task-rsvp_run-{args.run}_desc-coreg_boldref.nii.gz"

    # Fallback to T1w space BOLD if coreg_boldref not found
    if not bold_ref.exists():
        bold_ref = fmriprep_func_dir / f"sub-{args.subject}_task-rsvp_run-{args.run}_space-T1w_desc-preproc_bold.nii.gz"
        if not bold_ref.exists():
            print(f"❌ BOLD reference not found")
            return 1
    ...
    print("=" * 80)
    print("Native Space ROI Visualization")
    print("=" * 80)
    ...
    # Output paths
    overlay_png = subj_deriv_dir / f"QC_sub-{args.subject}_{args.roi}_run-{args.run}_detailed.png"
    hist_png = subj_deriv_dir / f"QC_sub-{args.subject}_{args.roi}_run-{args.run}_histogram.png"

    # Create visualizations
    print("📊 Creating overlay visualization...")
    create_roi_overlay(
        bold_ref, roi_mask, overlay_png,
        title=f"sub-{args.subject} {args.roi} ROI Overlay (Native BOLD Space)"
    )
    ...
    location = check_roi_location(roi_mask, bold_ref)
