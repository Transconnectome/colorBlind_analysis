#!/usr/bin/env python3
"""
Create HIGH RESOLUTION figure showing the 8 isoluminant color stimuli in CIE L*a*b* space
For A0 Poster use
"""
...
def create_stimulus_figure_highres():
    """
    Create HIGH RESOLUTION figure for A0 poster
    Larger fonts, thicker lines, higher DPI
    """
    # INCREASED figure size for better scaling
    ...
    ax.axhline(y=0, color='gray', linestyle='-', linewidth=3, alpha=0.5, zorder=1)
    ax.axvline(x=0, color='gray', linestyle='-', linewidth=3, alpha=0.5, zorder=1)

    # Draw concentric circles for reference - THICKER lines
    for radius in [20, 40]:
    ...
    # LARGER axis labels
    ax.set_xlabel('a*', fontsize=36, fontweight='bold')
    ax.set_ylabel('b*', fontsize=36, fontweight='bold')

    # LARGER tick labels
    ax.tick_params(axis='both', which='major', labelsize=24)

    # Plot neutral gray in center - LARGER circle
    gray_rgb = lab2rgb_accurate(54, 0, 0)  # L*=54, a*=0, b*=0
    gray_circle = Circle((0, 0), 6, facecolor=gray_rgb, edgecolor='black',
    ...
    for i in range(1, 9):
        ...
        # Plot color circle on theoretical position - LARGER circles
        color_circle = Circle((a_pos, b_pos), 7.5, facecolor=rgb,
                             edgecolor='black', linewidth=3.5, zorder=4)
        ax.add_patch(color_circle)

        # Add angle label outside - MUCH LARGER font
        label_offset = 1.35
    ...
    # Draw the perfect circle - THICKER line
    theta = np.linspace(0, 2*np.pi, 100)
    x_circle = radius * np.cos(theta)
    y_circle = radius * np.sin(theta)
    ax.plot(x_circle, y_circle, 'k-', linewidth=3.5, alpha=0.6, zorder=2)

    # Grid - THICKER
    ax.grid(True, alpha=0.2, linestyle=':', linewidth=1.5, zorder=0)

    plt.tight_layout()

    # Save figure with HIGH RESOLUTION
    output_path_png = 'Stimulus_Colors_Lab_Space_HighRes.png'
    output_path_pdf = 'Stimulus_Colors_Lab_Space_HighRes.pdf'

    # DPI 600 for A0 poster quality
    plt.savefig(output_path_png, dpi=600, bbox_inches='tight', facecolor='white')
    plt.savefig(output_path_pdf, bbox_inches='tight', facecolor='white')

    print(f"✓ HIGH RESOLUTION Figure saved:")
    print(f"  - {output_path_png} (DPI=600)")
    print(f"  - {output_path_pdf} (vector format)")
    ...
    print("="*70)
    print(f"  Figure size: 16 x 16 inches")
    print(f"  DPI: 600 (high resolution)")
    print(f"  Axis labels: fontsize=36")
    print(f"  Angle labels: fontsize=28")
    print(f"  Tick labels: fontsize=24")
    print(f"  Line widths: increased 2-3x")
    print(f"  Circle sizes: increased 1.5-2x")
    print("="*70)
...
if __name__ == '__main__':
    fig = create_stimulus_figure_highres()
    plt.show()
